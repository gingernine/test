	...
	\begin{screen}
		\begin{dfn}[Cartesian積]
			...
			\index{Cartesian せき@Cartesian 積}{\bf (Cartesian product)}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	\begin{prf}
		...
		\end{align}
		が成立する．そして$\chi$の任意性と
		論理的定理\ref{metathm:fundamental_law_of_universal_quantifier}から
		\begin{align}
	...
	\begin{prf}
		...
		論理的定理\ref{metathm:fundamental_law_of_universal_quantifier}より
		\begin{align}
			f(\sigma) = g(\sigma)
		\end{align}
		が満たされ，相等性の公理より
		...
		$\chi$の任意性と論理的定理\ref{metathm:fundamental_law_of_universal_quantifier}より
		\begin{align}
	...
	\begin{screen}
		\begin{dfn}[合成]
			...
			\begin{align}
				a \circ b \defeq
				\Set{x}{\exists s,t,u\, \left(\, x=(s,u) \wedge
				(s,t) \in b \wedge (t,u) \in a\, \right)}
			\end{align}
			で定める．
		\end{dfn}
	\end{screen}
	...
	\begin{screen}
		\begin{thm}[全射ならば原像の像で元に戻る]\label{projective_injective_image_preimage}
			...
			\begin{align}
				v \subset b \Longrightarrow f \ast \left(\inv{f} \ast v\right) \subset v
			\end{align}
			が成立し，特に$f$が全射なら
			\begin{align}
				f \ast \left(\inv{f} \ast v\right) = v
			\end{align}
			が成り立つ．
		\end{thm}
	\end{screen}
