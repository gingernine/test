...
\section{スコープ}
	いま$\varphi$を式とする．$\in$が$\varphi$に現れるとき，
	項$s$と項$t$が取れて，その$\in$の出現位置から
	\begin{align}
		\in st
	\end{align}
	なる原子式が$\varphi$の上に現れる．$\vee$が$\varphi$に現れるとき，
	式$\psi$と$\xi$が取れて，その$\vee$の出現位置から
	\begin{align}
		\vee \psi \xi
	\end{align}
	なる式が$\varphi$の上に現れる．$\exists$が$\varphi$に現れるとき，
	項$x$と式$\psi$が取れて
	\begin{align}
		\exists x \psi
	\end{align}
	...
	これは直感的に正しいことであるが，直感を排除してこれを認めるには構造的帰納法の原理が必要になる．
...
\section{式の書き換え}
	...
	\begin{screen}
		\begin{logicalaxm}
			\begin{align}
				\exists x A(x) \leftrightarrow A(\varepsilon x \hat{A}(x)).
			\end{align}
