\section{測度}
	\subsection{Lebesgue拡大}
		...
		\begin{screen}
			\begin{lem}[距離空間値の可測関数列の各点極限は可測]\label{lem:measurability_metric_space}
				$(S,d)$を距離空間，$(X,\mathcal{B})$を可測空間とする．
				$\mathcal{B}/\borel{S}$-可測関数列$(f_n)_{n=1}^{\infty}$が
				各点収束すれば，
				$f \coloneqq \lim_{n \to \infty} f_n$で定める関数$f$もまた可測$\mathcal{B}/\borel{S}$となる．
			\end{lem}
		\end{screen}
		...
		\begin{screen}
			\begin{lem}[複素数値関数の可測性]\label{thm:measurability_of_complex_measurable_functions}
				...
				$f$の実部$u$と虚部$v$がそれぞれ$\mathcal{M}/\borel{\R}$-可測であることは同値である．
			\end{lem}
		\end{screen}
