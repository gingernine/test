	...
	\subsection{有限加法的測度の拡張}
		...
		\begin{screen}
			\begin{thm}[完全加法性の同値条件]
				...
				とおくとき，
				\begin{description}
					\item[(1)] $\mu(X) < \infty$なら$\mu$が$\mathcal{B}$の上で
		...
		\begin{prf}
			定理\ref{thm:Cantor_intersection_theorem}より従う．
			\QED
		\end{prf}
		
		\begin{screen}
			\begin{thm}[コンパクトクラスの存在と共通点性]\label{thm:compact_class_intersection}
				$\mathcal{B}$を集合$X$の上の有限加法族，
				...
				減少列$\{B_n\}_{n=1}^\infty \subset \mathcal{B}$に対し次が満たされる:
				\begin{align}
					\mu(B_1) < \infty,
					\quad \bigcap_{n=1}^\infty B_n = \emptyset
		...
		\begin{prf}
			$(B_n)_{n=1}^\infty$を$\mu(B_1) < \infty,\ \bigcap_{n=1}^\infty B_n = \emptyset$
			を満たす減少列とすれば，任意の$\epsilon > 0$に対して
			\begin{align}
