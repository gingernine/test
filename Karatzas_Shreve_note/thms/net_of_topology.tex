...
\subsection{Dynkin族定理}
	...
	Dynkin族の定義(D3)が単調増大な集合列で閉じていることになっていることもあるから次を載せておく．
	\begin{screen}
		\begin{thm}[可算直和で閉じることと増大列の可算和で閉じることの同値性]
			集合$X$の部分集合族$\mathscr{D}$が
			の定義\ref{def:Dynkin_system_theorem}の(D1),(D2)を満たしているとき，
			$\mathscr{D}$が(D3)を満たすことと
			$\mathscr{D}$が可算直和で閉じることは同値である．
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		...
		\end{align}
		が成立する．逆に$\mathscr{D}$が(D3)を満たしているとする．
		...
		が得られる．よって
		\begin{align}
			D_n \coloneqq \bigcup_{i=1}^n B_i = X \backslash \Biggl( \bigcap_{i=1}^n B_i^c \Biggr) \in \mathscr{D},
			\quad (n=1,2,\cdots)
		\end{align}
