...
\subsection{分離公理}
	...
	\begin{screen}
		\begin{thm}[Hausdorff空間においてコンパクト部分集合は閉]
			$S$をHausdorff空間，
		\end{thm}
	\end{screen}
	
	\begin{screen}
		\begin{thm}[Hausdorff空間においてコンパクト部分集合は]
		\end{thm}
	\end{screen}
	
	\begin{screen}
		\begin{thm}[正則空間は閉近傍系が基本近傍系となる空間]
		\label{thm:each_point_in_regular_space_has_closesd_local_base}
			$S$を位相空間とするとき，$S$が正則であることと，任意のコンパクト部分集合$K \subset S$と
			開集合$U \subset S$に対し$K \subset U$なら
			\begin{align}
			...
			また，$S$が局所コンパクトな正則空間であるとき，
			(\refeq{eq:thm_each_point_in_regular_space_has_closesd_local_base})において
			$\overline{V}$がコンパクトとなるように開集合$V$を取れる．
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		...
		\end{align}
		となるから(\refeq{eq:thm_each_point_in_regular_space_has_closesd_local_base})が成立する．
	...
	\begin{screen}
		\begin{thm}[正規なら交わらない二つの閉集合は関数で分離される．(Urysohnの補題)]
			$S$を正規空間，$A,B$を$S$の閉集合とするとき，次を満たす連続関数$f:S \longrightarrow \R$が存在する:
			\begin{align}
				f(A) = \{0\}, \quad f(B) = \{1\}.
			\end{align}
		\end{thm}
...
\subsection{位相的補題}
	...
	\begin{screen}
		\begin{thm}[局所コンパクト空間において$T_2$と$T_3$は同値]\label{thm:regularity_of_Hausdorff_spaces}
			局所コンパクトHausdorff空間は$T_3$である．
		\end{thm}
	\end{screen}
	
	\begin{prf}
		$S$を局所コンパクトHausdorff空間とし，点$x \in S$と閉集合$F \subset S$が$x \notin F$を満たしているとする．
