...
\subsection{分離公理}
	...
	\begin{screen}
		...
		\label{thm:each_point_in_regular_space_has_closesd_local_base}
			$S$を位相空間とするとき，$S$が正則であることと，任意の$x \in S$と開集合$U \subset S$に対し
			$x \in U$なら
			\begin{align}
	...
	\begin{prf}
		...
		\begin{align}
			x \in V_1,\ \quad X \backslash U \subset V_2,
			\quad V_1 \cap V_2 = \emptyset
		\end{align}
		を満たす開集合$V_1,V_2$が存在する．このとき$X \backslash V_2$は$V_1$を含む閉集合であり
		\begin{align}
			x \in V_1 \subset \overline{V_1} \subset X \backslash V_2 \subset U
		\end{align}
		となるから(\refeq{eq:thm_each_point_in_regular_space_has_closesd_local_base})が成立する．
		逆に後者が満たされているとき，任意の$y \in S$と閉集合$F \subset S$に対し，$y \notin F$なら
		\begin{align}
			y \subset G \subset \overline{G} \subset X \backslash F
		\end{align}
