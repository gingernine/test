...
\subsection{距離空間}
	\begin{screen}
		\begin{dfn}[(擬)距離関数・距離位相]
			空でない集合$S$において，
			\begin{description}
				\item[(非負性)] $d(x,y) \geq 0,\quad (\forall x,y \in S)$
				\item[(対称性)] $d(x,y) = d(y,x),\quad (\forall x,y \in S)$
				\item[(三角不等式)] $d(x,y) \leq d(x,z) + d(z,y),\quad (\forall x,y,z \in S)$
			\end{description}
	...
	\begin{prf}
		任意に$x,y \in S,\ (x \neq y)$を取る．また$r > 0$に対し
		$B_r(x) \coloneqq \Set{s \in S}{d(x,s) < \epsilon}$と書く．このとき
		\begin{align}
			&\mbox{$d$が距離} \quad \Longrightarrow \quad
			\epsilon \coloneqq d(x,y)> 0 \quad \Longrightarrow \quad
			y \in S \backslash B_\epsilon(x) \\ 
			&\qquad \quad \Longrightarrow \quad
			\overline{\{y\}} \subset S \backslash B_\epsilon(x)
			\quad \Longrightarrow \quad
			x \notin \overline{\{y\}}
			\quad \Longrightarrow \quad
			\mbox{擬距離位相が$T_0$}
		\end{align}
		が成立し，逆に擬距離位相が$T_0$なら
		$x \notin \overline{\{y\}}$か$y \notin \overline{\{x\}}$となるから，
	...
	\begin{prf}
		...
		\end{align}
		が満たされるから$S$は完全正規である．また$d$が距離であるとき，相異なる二点$x,y$に対し
		\begin{align}
		...
		\end{align}
		で交わらない開球を定めれば$x,y$は$B_\epsilon(x),\ B_\epsilon(y)$で分離され，
		$S$はHausdorffかつ完全正規となるから$T_6$となる．
		逆に$S$が$T_6$空間であれば$T_0$が満たされるから，
		定理\ref{thm:pseudometric_is_metric_iff_T_0}より$d$は距離となる．
		\QED
	\end{prf}
