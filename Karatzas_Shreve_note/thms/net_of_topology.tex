...
\subsection{位相}
	...
	\begin{screen}
		\begin{thm}[基本近傍系は開集合を決定する]\label{thm:local_base_defines_open_sets}
			...
			\end{align}
			が成立する．すなわち，$\{\mathscr{U}(x)\}_{x \in S}$を基本近傍系とする位相は唯一つである．
		\end{thm}
	\end{screen}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[(1)] 任意の$U \in \mathscr{U}(x)$は$x$の近傍であるから
				$x \in U^{\mathrm{o}}$となり$(LB1)$が満たされる．
				また$U,V \in \mathscr{U}(x)$に対し
				\begin{align}
...
\subsection{分離公理}
	...
	\begin{screen}
		\begin{thm}[Hausdorff空間とは交わらない二つのコンパクト集合が近傍で分離できる空間]
		\end{thm}
	\end{screen}
