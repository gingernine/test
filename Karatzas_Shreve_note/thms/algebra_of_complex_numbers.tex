\subsection{複素数の代数的性質}
	...
	\begin{align}
		z = x + \isym \cdot y
	\end{align}
	を満たす$\R^2$の要素$(x,y)$が唯一つ取れる．この$+$と$\cdot$はまさに上で述べた加法と乗法である．
	\begin{align}
		\R^2 \ni (x,y) \longmapsto x + \isym \cdot y \in \C
	\end{align}
	なる関係は$\R^2$から$\C$への全単射であり，上の$z$に対する$x$を
	\begin{align}
		\Re{z}
	\end{align}
	と書いて$z$の{\bf 実部}{\bf (real part)}と呼び，$z$に対する$y$を
	\begin{align}
		\Im{z}
	\end{align}
	と書いて$z$の{\bf 虚部}{\bf (imaginary part)}と呼ぶ．特に虚部が$0$である複素数は実数であるから
	$\R$は$\C$の部分集合である:
	\begin{align}
		\R \subset \C.
	\end{align}
	また$\C$上の加法と乗法といった算法は$\R$上の加法と乗法の純粋な延長である
	\footnote{
		以上の関係が成り立つように数を構成できるということであるが，そのように書いていない文献が案外多いので注意が必要である．
		$\C = \R^2$と考えるのは危険で，複素数と実数の順序対は一対一の対応があるが別物である．
		$\R^2$に相応の算法を導入しただけでは$\R$は$\C$の部分集合になり得ない．
	}．
	$(\C,+,\cdot\ )$の組は複素数体と呼ばれる閉じた世界である．$\R$も$\C$と同様に加法と乗法で閉じていて，
	...
	\begin{sketch}
		...
		\end{align}
		が成り立つが，他方で定理\ref{thm:C_has_no_zero_divisor}から
		\begin{align}
			(a + b) \cdot (a - b) = 0
