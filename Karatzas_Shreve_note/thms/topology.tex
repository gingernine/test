\section{位相}
	...
	いま$a$と$b$には位相構造が定まっているので，$x$と$f(x)$のそれぞれに対して``近所''というものが把握できるわけであるが，
	ここで$f(x)$の近所をどんなに``狭く''取ったとしても，$x$の近所で，そこの任意の要素$y$で
	\begin{align}
		f(y) \in \mbox{$f(x)$の近所}
	\end{align}
	となるものが取れるとする．つまりイメージとしては，$x$の近所をどんどん狭く取って$f$で移していくと，
	その像は$f(x)$に``収束''していく．このとき，$f$の値は$f(x)$の近くで``ズレが抑えられていく''から，
	まるで$f(x)$の周りで値が``連続的''に繋がっているという意味で``$f$は$x$で連続である''と言われる．
	...
	\begin{screen}
		\begin{dfn}[開集合と閉集合]
			...
			\end{align}
			なる$S$の部分集合$a$を$(S,\mathscr{O})$の{\bf 閉集合}\index{へいしゅうごう@閉集合}{\bf (closed set)}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	\begin{screen}
		\begin{dfn}[内部・閉包]
			...
			$A$の$\mathscr{O}$に関する{\bf 内部}\index{ないぶ@内部}{\bf (interior)}と呼び，
	...
	\begin{align}
		v \subset A^{\mathrm{o}}
	\end{align}
	が成立する．つまり，{\bf $A$の$\mathscr{O}$に関する内部は，$A$に含まれる$(S,\mathscr{O})$の開集合のうちで包含関係に関して最大のものである．}
	...
	\begin{align}
		\overline{A} \subset v
	\end{align}
	が成立する．つまり，{\bf $A$の$\mathscr{O}$に関する閉包は，$A$を含む$(S,\mathscr{O})$の閉集合のうちで包含関係に関して最小のものである．}
	
	\begin{screen}
		\begin{thm}[内部の補集合は補集合の閉包]
		\label{thm:topology_note_closure_interior}
			$(S,\mathscr{O})$を位相空間とし，$A$を$S$の部分集合とする．また
			\begin{align}
				A^c \defeq S \backslash A
			\end{align}
			と書き，$A$の$\mathscr{O}$に関する内部を
			\begin{align}
				A^i
			\end{align}
			と書き，$A$の$\mathscr{O}$に関する閉包を
			\begin{align}
				A^a
			\end{align}
			と書き，$(A^i)^c$など連鎖する場合は括弧を省略して$A^{ic}$と書く．このとき次が成り立つ．
			\begin{itemize}
				\item $A^{ic} = A^{ca}.$
				\item $A^{cic} = A^a.$
				\item $A^{ci} = A^{ac}.$
