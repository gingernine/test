\subsection{位相}
		...
		\begin{dfn}[被覆・コンパクト・局所コンパクト・$\sigma$-コンパクト]\mbox{}
			\begin{itemize}
				\item
					...
					$S$の被覆\index{ひふく@被覆}(cover)であるとは，
					\begin{align}
						S = \bigcup \mathscr{B}
					\end{align}
	...
	\begin{screen}
		\begin{dfn}[有限交叉性]
			...
			$\bigcap \mathscr{T} \neq \emptyset$を満たすとき
			$\mathscr{S}$は有限交叉性\index{ゆうげんこうさせい@有限交叉性}
			(finite intersection property)を持つという．
		\end{dfn}
	\end{screen}
...
\subsection{可算公理}
	...
	\begin{screen}
		...
		\label{thm:locally_finite_family_of_open_sets_is_countable_in_separable_space}
			...
			\end{align}
			が成立する．すなわちこのとき$\mathscr{B}$は高々可算集合である．
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		...
		\end{align}
		で高々可算集合$\mathscr{U}$が定まる．稠密性より任意の$E \in \mathscr{B}$は
