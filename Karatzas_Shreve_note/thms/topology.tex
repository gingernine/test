...
\subsection{分離公理}
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[(2)]
				...
				となる開近傍$U_x$とコンパクトな近傍$C_x$が存在するから，
				\begin{align}
					K \subset (C_{y_1}^{\mathrm{o}} \cap U_{y_1}) 
					\cup \cdots \cup (C_{y_m}^{\mathrm{o}} \cap U_{y_m})
				\end{align}
				を満たす$\{y_i\}_{i=1}^m \subset K$に対し
				$U \coloneqq \bigcup_{i=1}^m C_{y_i}^{\mathrm{o}} \cap U_{y_i}$
				とおけば，
				\begin{align}
					\overline{U} \subset \bigcup_{i=1}^m C_{y_i}
				\end{align}
				と定理\ref{thm:closed_subset_of_compact_set_is_compact_on_Hausdorff_space}より
				$\overline{U}$はコンパクトであり
	...
	\begin{screen}
		\begin{thm}[$T_2$なら$T_1$]
		\end{thm}
	\end{screen}
	\begin{screen}
		\begin{thm}[$T_3$なら$T_2$]
		\end{thm}
	\end{screen}
	\begin{screen}
		\begin{thm}[$T_4$なら$T_3$]
		\end{thm}
	\end{screen}
	\begin{screen}
		\begin{thm}[$T_5$なら$T_4$]
		\end{thm}
	\end{screen}
	\begin{screen}
		\begin{thm}[$T_6$なら$T_5$]
		\end{thm}
	\end{screen}
...
\subsection{可算公理}
	...
	\begin{screen}
		\begin{dfn}[局所有限]
			...
			{\bf (locally finite)}であるという．また
			$\mathscr{F}$が局所有限な部分集合族の可算和で表されるとき，
	...
	\begin{screen}
		\begin{thm}[正規かつ$\sigma$-局所有限な基底を持つ$\Longleftrightarrow$完全正規]
		\end{thm}
	\end{screen}
