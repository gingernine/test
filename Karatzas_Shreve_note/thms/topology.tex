\subsection{位相}
	\begin{screen}
		\begin{dfn}[位相]
			...
			以下を満たすとき，$\mathscr{O}$を$S$の{\bf 位相}
			\index{いそう@位相}{\bf (topology)}，或は{\bf 開集合系}
			...
			\end{description}
			また$\mathscr{O}$の元を$S$の{\bf 開集合}
			\index{かいしゅうごう@開集合}{\bf (open set)}と呼び，
			補集合が開である集合を{\bf 閉集合}\index{へいしゅうごう@閉集合}
			{\bf (closed set)}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	の位相となる．以後$\R$と$\C$には暗黙の裡にこの位相が入る．
