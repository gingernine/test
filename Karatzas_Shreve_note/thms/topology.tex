...
\subsection{分離公理}
	...
	\begin{screen}
		\begin{thm}[完全正則空間の位相は実連続写像全体の始位相に一致する]
			$(S,\mathscr{O})$を空でない位相空間，$C(S)$を実連続写像の全体とするとき，
			\begin{align}
				\mbox{$S$が完全正則} \quad \Longleftrightarrow \quad
				\mbox{$C(S)$-始位相が$\mathscr{O}$に一致する}.
			\end{align}
		\end{thm}
	\end{screen}
	
	\begin{prf}
		$C(S)$-始位相を$\mathscr{O}_{IT}$と書く．
		\begin{align}
			\bigcup_{f \in C(S)} \Set{f^{-1}(O)}{\mbox{$O$は$\R$の開集合}}
		\end{align}
		は$\mathscr{O}_{IT}$を生成し，かつ$\mathscr{O}$に含まれるから
		$\mathscr{O}_{IT} \subset \mathscr{O}$を得る．
		$S$が完全正則であるとき，空でない$O \in \mathscr{O}$を取れば任意の$x \in O$に対して
		\begin{align}
			f(x) = 0,\quad f(y) = 1,\ (\forall y \in S \backslash O)
		\end{align}
		を満たす$f \in C(S)$が存在するが，
		このとき$B_1 \coloneqq \Set{\alpha \in \R}{|\alpha| < 1}$とおけば
		\begin{align}
			f^{-1}(B_1) \subset O
		\end{align}
		が成り立つ．$f^{-1}(B_1)$は$\mathscr{O}_{IT}$における$x$の開近傍であるから
		$O \in \mathscr{O}_{IT}$が従い$\mathscr{O}_{IT} \subset \mathscr{O}$が成立する．
	\end{prf}
