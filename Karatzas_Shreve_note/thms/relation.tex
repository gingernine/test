\subsection{関係}
	...
	と定めたら
	\begin{align}
		\fnc{a} \Longleftrightarrow \rel{a} \wedge 
		\forall s,t \in \dom{a}\, (\, s=t \Longrightarrow a(s) = a(t)\, )
	\end{align}
	が言える?
	$\Longrightarrow$なら言える．OK.
	逆は怪しい．蓋しNG.
	
	\begin{screen}
		\begin{dfn}[写像] $f,a,b$を類とするとき，以下の概念と$\mathcal{L}'$における派生記号を定める．
			\begin{itemize}
				\item $f$が\index{single-valued}{\bf single-valuedである}ということ:
					\begin{align}
						\sing{f} \overset{\mathrm{def}}{\Longleftrightarrow}
						\forall x,y,z\, (\, (x,y) \in f \wedge (x,z) \in f
						\Longrightarrow y=z\, ).
					\end{align}
	...
	\begin{screen}
		\begin{dfn}[像・原像]
			$a,b$を類とするとき
			\begin{align}
				a \ast b \coloneqq \Set{x}{\exists t \in b\, (\, x=a(t)\, )} 
			\end{align}
		\end{dfn}
	\end{screen}
