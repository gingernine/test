\subsection{関係}
	...
	\begin{screen}
		\begin{dfn}[定義域・値・値域]
			$a$を類とするとき，
			\begin{itemize}
				\item $\operatorname{dom}(a) \coloneqq \Set{x}{\exists y\ (\ (x,y) \in a\ )}$
				\item $\operatorname{ran}(a) \coloneqq \Set{y}{\exists x\ (\ (x,y) \in a\ )}$					
			\end{itemize}
	...
	\begin{screen}
		\begin{dfn}[single-valued・写像]
			類$a$に対して
			\begin{align}
				\operatorname{Sing}(a) \overset{\mathrm{def}}{\Longleftrightarrow}
				\forall x,y,z\ (\ (x,y) \in a \wedge (x,z) \in a
				\Longrightarrow y=z\ )
			\end{align}
			で$\operatorname{Sign}(a)$を定め，$\operatorname{Sign}(a)$が成り立つとき
			$a$は\index{single-valued}{\bf single-valuedである}という．また類$a$に対して
			\begin{align}
				\operatorname{Fnc}(a) \overset{\mathrm{def}}{\Longleftrightarrow}
				a \subset \Univ \times \Univ \wedge \operatorname{Sing}(a)
			\end{align}
			で$\operatorname{Sign}(a)$を定め，$\operatorname{Sign}(a)$を満たす類$a$を
