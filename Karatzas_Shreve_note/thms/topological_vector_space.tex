...
\subsection{線型位相}
	...
	\begin{screen}
		...
		\label{thm:equicontinuity_on_topological_vector_spaces}
			...
			\begin{align}
				\mathscr{V}_X &\coloneqq \Set{V}{\exists B \in \mathscr{B}_X\, \left(\, 
					\Set{(x,y)}{x,y \in X \wedge y-x \in B} \subset V\, \right)}, \\
				\mathscr{V}_Y &\coloneqq \Set{V}{\exists B \in \mathscr{B}_Y\, \left(\, 
	...
	\begin{sketch}
		...
		\begin{align}
			V_B \coloneqq \Set{(x,y)}{x,y \in Y \wedge y - x \in B}
		\end{align}
		...
		\begin{align}
			U \coloneqq \Set{(x,y)}{x,y \in X \wedge y - x \in C}
		\end{align}
		とおくと
		\begin{align}
