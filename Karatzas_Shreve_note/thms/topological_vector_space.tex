...
\subsection{線型位相}
	...
	\begin{screen}
		\begin{thm}[平行移動・スカラ倍は連続]\label{thm:continuity_of_translations_multiples}
			...
			\begin{description}
				...
				\item[(2)] 任意の$\beta \in \Phi$に対し
					$X \ni x \longmapsto \beta x \in X$は連続であり，
					特に$\beta \neq 0$のとき同相写像となる．
					
				\item[(3)] 任意の$a \in X$に対し
					$\Phi \ni \alpha \longmapsto \alpha a \in X$は連続である．
			\end{description}
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		...
		また，(1)(2)において$x \longmapsto -a+x,\ x \longmapsto \beta^{-1}x$
		が逆写像となる．
		\QED
	\end{prf}
	...
	\begin{screen}
		\begin{thm}[ノルムで定める距離は唯一の斉次的かつ平行移動不変な距離]
			ノルムで導入する距離は斉次的かつ平行移動不変であり，逆に
			斉次的かつ平行移動不変な距離はノルムである．
		\end{thm}
	\end{screen}
