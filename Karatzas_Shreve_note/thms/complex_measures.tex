\subsection{複素測度}
	\begin{screen}
		\begin{dfn}[複素測度]
			$(X,\mathscr{F})$を可測空間とするとき，$\mathscr{F}$で定義される完全加法的な複素数値関数を
			$(X,\mathscr{F})$上の{\bf 複素測度}\index{ふくそそくど@複素測度}{\bf (complex measure)}という．
		\end{dfn}
	\end{screen}
	
	$\lambda$を可測空間$(X,\mathscr{F})$の複素測度とする．
	任意の全単射$\sigma:\Natural \rightarrow \Natural$に対し
	\begin{align}
	...
	\begin{prf} $|\lambda|(X) = \infty$と仮定して背理法により定理を導く．
		...
		\item[第一段]
			...
			\end{align}
			を満たす$E$の分割$(E_i)_{i=1}^{\infty}$が存在する．従って或る$N \in \Natural$に対し
			\begin{align}
	...
	\begin{prf}
		...
		(\refeq{eq:thm_complex_measure_finite_splits})と併せて
		\begin{align}
			|\lambda|(E) = \sup{}{\Set{\sum_{i=1}^n|\lambda(A_i)|}{E=\sum_{i=1}^n A_i,
			\ \{A_i\}_{i=1}^n \subset \mathscr{F},\ n=1,2,\cdots}}
		\end{align}
		が得られる．$|\lambda|(E) = 0$なら(\refeq{eq:thm_complex_measure_finite_splits})で
		等号成立となる．
		\QED
	\end{prf}
