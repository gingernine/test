\subsection{距離空間}
	...
	\begin{prf}
		...
		\end{align}
		が従い(\refeq{eq:thm_base_on_pseudometric_space_1})が出る．
	...
	\begin{screen}
		\begin{thm}[擬距離空間は$\sigma$-局所有限な基底を持つ]
			$(X,d)$を擬距離空間，$\mathscr{X}$を$X$の任意の被覆とするとき，
			$\mathscr{X}$の開細分で$\sigma$-局所有限であるものが存在する．
			特に，$\mathscr{X}$が$X$の擬距離位相の基底であれば
			その$\sigma$-局所有限な開細分も基底となるから，
			擬距離空間は$\sigma$-局所有限な基底を持つ．
		\end{thm}
	\end{screen}
	
	\begin{prf}
		整列可能定理により$\mathscr{X}$を整列集合にする全順序$\leq$が存在する
		(ただし異なる二元$U,V$が$U \leq V$となるときは$U < V$と書く)．
		任意の$U \in \mathscr{X}$に対し
		\begin{align}
			I_n(U) &\coloneqq \Set{x \in X}{d(x,U^c) \geq \frac{1}{n}}, \\
			J_n(U) &\coloneqq I_n(U) \cap \Biggl(\bigcup_{V<U}V\Biggr)^c, \\
		\end{align}
	\end{prf}
