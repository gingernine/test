\subsection{距離空間}
	...
	\begin{screen}
		\begin{thm}[擬距離空間は$\sigma$-局所有限な基底を持つ]
		\label{thm:open_cover_on_pseudometric_space_has_sigma_locally_finite_refinement}
			$(X,d)$を擬距離空間，$\mathscr{X}$を$X$の任意の開被覆とするとき，
			$\mathscr{X}$の開細分で$\sigma$-局所有限であるものが存在する．
			特に，擬距離空間は$\sigma$-局所有限な基底を持つ．
		\end{thm}
	\end{screen}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				整列可能定理により$\mathscr{X}$を整列集合にする全順序$\preceq$が存在する．ここで
				\begin{align}
					U \prec V \quad \overset{\mathrm{def}}{\Longleftrightarrow} \quad
					\mbox{$U \preceq V$かつ$U \neq V$}
				\end{align}
				とする．中心$x$で半径$r$の開球を$B_r(x)$と書き，
				任意の$n \in \N$と$U \in \mathscr{X}$に対し
				\begin{align}
					I_n(U) &\coloneqq \Set{x \in X}{B_{1/n}(x) \subset U},
					\quad (I_n(\emptyset) = \emptyset), \\
					J_n(U) &\coloneqq I_n(U) \left\backslash \bigcup_{V \prec U}V \right.
				\end{align}
				として$\mathscr{X}_n \coloneqq \Set{U \in \mathscr{X}}{J_n(x) \neq \emptyset}$
				とおく．$\mathscr{X}$の$\preceq$に関する最小元を$V$として
				$x \in V$を一つ取れば，$V$は開集合であるから十分大きい$n_0$で
				$B_{1/n_0}(x) \subset V$となり，$V$の最小性より
				\begin{align}
					x \in I_{n_0}(V) = J_{n_0}(V)
				\end{align}
				が成り立つので少なくとも$n \geq n_0$ならば$\mathscr{X}_n$は空でない．
				$\mathscr{X}_n \neq \emptyset$のとき，任意の$U \in \mathscr{X}_n$に対し
				\begin{align}
					K_n(U) \coloneqq \bigcup_{x \in J_n(U)} B_{1/(3n)}(x)
				\end{align}
				により開集合を定めて$\mathscr{K}_n \coloneqq 
				\Set{K_n(U)}{U \in \mathscr{X}_n}$とおけば次が成立する:
				\begin{description}
					\item[(1)] 任意の$U \in \mathscr{X}_n$に対し$K_n(U) \subset U$となる．
					
					\item[(2)] 相異なる二元$U, V \in \mathscr{X}_n$に対し
						\begin{align}
							d(x,y) \geq \frac{1}{3n},\quad
							(\forall x \in K_n(U),\ y \in K_n(V)).
						\end{align}
						
					\item[(3)] 半径$1/(6n)$の任意の開球は，$\mathscr{K}_n$の二個以上の元と
						交わることはない．
				\end{description}
				実際，任意の$x \in K_n(U)$は或る$x_0 \in J_n(U)$を中心とする半径$1/(3n)$の開球に入るから
				\begin{align}
					x \in B_{1/(3n)}(x_0) \subset B_{1/n}(x_0) \subset U 
				\end{align}
				となり(1)が出る．また$U, V \in \mathscr{X}_n$が
				$U \neq V$であるとき，$U \prec V$か$V \prec U$の一方が成り立つ．
				$V \prec U$が成り立つとき，任意に$x \in K_n(U)$と$y \in K_n(V)$を取れば
				或る$x_0 \in J_n(U)$と$y_0 \in J_n(V)$で
				\begin{align}
					d(x_0,x) < \frac{1}{3n}, \quad d(y_0,y) < \frac{1}{3n}
				\end{align}
				となる．一方で$J_n$の定義より$x_0 \notin V$かつ$y_0 \in V$が満たされるから
				$d(x_0,y_0) \geq 1/n$が従い，
				\begin{align}
					d(x,y) \geq d(x_0,y_0) - d(x,x_0) - d(y,y_0) \geq \frac{1}{n}
					- \frac{1}{3n} - \frac{1}{3n} = \frac{1}{3n}
				\end{align}
				が成り立ち(2)も出る．これにより半径$1/(6n)$の任意の開球は
				$\mathscr{K}_n$の二個以上の元と交わり得ない．ここで
				\begin{align}
					\mathscr{K} \coloneqq 
					\bigcup_{\substack{n \in \N \\ \mathscr{X}_n \neq \emptyset}} \mathscr{K}_n
				\end{align}
				とおけば，$\mathscr{K}$は$\mathscr{X}$の$\sigma$-局所有限な開細分となる．実際
				(1)より$\mathscr{K}$の元は全て$\mathscr{X}$の元の部分集合であり，(3)より
				各$\mathscr{K}_n$は局所有限である．また
				任意の$x \in X$に対し，$x$を含む$\mathscr{X}$の元のうち$\preceq$に関する最小元を
				$V$とすれば，十分大きい$n$で$B_{1/n}(x) \subset V$となるから$x \in I_n(V)$
				が従い，最小性より$W \prec V$なら$x \notin W$が満たされ
				\begin{align}
					x \in J_n(V) \subset K_n(V)
				\end{align}
				が成立する．すなわち$\mathscr{K}$は$X$を覆っている．
		
			\item[第二段]
				$X$の開被覆$\mathscr{B}_n \coloneqq \Set{B_{1/n}(x)}{x \in S}$
				に対して$\sigma$-局所有限な開細分$\mathscr{A}_n$が存在し，
				任意の$A \in \mathscr{A}_n$で
				\begin{align}
					d(x,y) < \frac{2}{n},\quad
					(\forall x,y \in A)
					\label{eq:thm_open_cover_on_pseudometric_space_has_sigma_locally_finite_refinement}
				\end{align}
				が満たされる($n=1,2,\cdots$)．いま，
				任意に開集合$U$と$x \in U$を取れば或る$n \in \N$で
				\begin{align}
					B_{1/n}(x) \subset U
				\end{align}
				となる．一方で$\mathscr{A}_{1/(2n)}$が$X$を覆うから
				或る$A \in \mathscr{A}_{1/(2n)}$もまた$x$を含み，このとき
(\refeq{eq:thm_open_cover_on_pseudometric_space_has_sigma_locally_finite_refinement})より
				\begin{align}
					x \in A \subset B_{1/n}(x)
				\end{align}
				が成立する．従って$\mathscr{A} \coloneqq \bigcup_{n \in \N}\mathscr{A}_n$
				は$X$の基底となり，各$\mathscr{A}_n$が$\sigma$-局所有限であるから
				$\mathscr{A}$も$\sigma$-局所有限である．
				すなわち$X$は$\sigma$-局所有限な基底を持つ．
				\QED
		\end{description}
	\end{prf}
