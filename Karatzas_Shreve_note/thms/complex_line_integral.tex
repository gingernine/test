\subsection{複素線積分}
	...
	\begin{screen}
		\begin{dfn}[路]
			$\R$の有界閉区間上で定義された連続で有界変動な$\C$値関数を{\bf 路}\index{ろ@路}{\bf (contour)}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	\begin{itembox}[l]{線積分のパラメータ区間変項}
		...
		\end{align}
		の関係で定める$[\sigma,\tau]$上の写像とする．このとき$\gamma \circ \varphi$は$[\sigma,\tau]$上の連続な有界変動関数であって，
		$\borel{[\alpha,\beta]}$の任意の要素$E$で
		\begin{align}
			\mu_{\gamma}(E) = \mu_{\gamma \circ \varphi} \left(\varphi^{-1} \ast E\right).
		\end{align}
	\end{itembox}
	
	これが示されれば，$[\alpha,\beta]$上の任意の$\mathscr{O}_{[\alpha,\beta]}/\mathscr{O}_{\C}$-連続写像$g$に対して
	\begin{align}
		\int_{[\alpha,\beta]} g\ d\mu_{\gamma} = \int_{[\sigma,\tau]} g \circ \varphi\ d\mu_{\gamma \circ \varphi}
		\label{fom:change_of_parameter_interval_complex_contour_integral}
	\end{align}
	が成立する．実際，$g$が$\borel{[\alpha,\beta]}/\borel{\C}$-可測単関数である場合は
	\begin{align}
		\int_{[\alpha,\beta]} g\ d\mu_{\gamma} = \int_{[\sigma,\tau]} g \circ \varphi\ d\mu_{\gamma \circ \varphi}
	\end{align}
	が成り立つから，Lebesgueの収束定理より$g$が一般の$\mathscr{O}_{[\alpha,\beta]}/\mathscr{O}_{\C}$-連続写像である場合も
	\begin{align}
		\int_{[\alpha,\beta]} g\ d\mu_{\gamma} = \int_{[\sigma,\tau]} g \circ \varphi\ d\mu_{\gamma \circ \varphi}
	\end{align}
		...
		\begin{description}
			\item[第一段]
				...
				\begin{align}
					...
					\leq v
				\end{align}
				が成立する．これは$[\sigma,\tau]$の分割の取り方に依らないから$\gamma \circ \varphi$は有界変動である．
				$\gamma \circ \varphi$の連続性は$\gamma$と$\varphi$の連続性から従う．
			...
			\item[第二段]
				...
				\begin{align}
					\mu_{\gamma \circ \varphi}\left(\varphi^{-1} \ast ]s,t]\right)
					&= \mu_{\gamma \circ \varphi}\left(\left]\varphi^{-1}(s), \varphi^{-1}(t)\right]\right) \\
					&= \gamma \left(\varphi \left(\varphi^{-1}(t) \right) \right)
					- \gamma \left(\varphi \left(\varphi^{-1}(s) \right) \right) \\
					&= \gamma(t) - \gamma(s) \\
					&= \mu_{\gamma}(]s,t])
				\end{align}
				が成立する．$\mu_{\gamma}$も$\mu_{\gamma \circ \varphi}$も一点の測度は$0$であるから，
				測度の一致の定理より
				\begin{align}
					\mu_{\gamma}(E) = \mu_{\gamma \circ \varphi} \left(\varphi^{-1} \ast E\right)
				\end{align}
	...
	\begin{screen}
		...
		\label{thm:consistency_of_contour_integral_on_sub_interval}
			...
			\begin{align}
				\int_{[\sigma,\eta]} f\ d\mu_{\gamma} = \int_{[\sigma,\tau]} \rest{f}{[\sigma,\tau]}\ d\mu_{\rest{\gamma}{[\sigma,\tau]}}.
			\end{align}
		\end{thm}
	\end{screen}
	...
	\begin{sketch}
		便宜上
		\begin{align}
			\eta \defeq \rest{\gamma}{[\sigma,\tau]}
		\end{align}
		とおくとき
		\begin{align}
			\rest{\mu_{\gamma}}{\borel{[\sigma,\tau]}} = \mu_{\eta}
		\end{align}
		が成り立つ．実際，$s$と$t$を
		\begin{align}
			s < t
