\subsection{開基}
	...
	\begin{screen}
		\begin{dfn}[開基]
			...
			\end{align}
			を満たすものを$\mathscr{O}$-{\bf 開基}\index{かいき@開基}{\bf (open base)}や
			$\mathscr{O}$-{\bf 基底}\index{きてい@基底}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	\begin{screen}
		\begin{dfn}[Cartesian積の位相]
			$(S,\mathscr{O}_{S})$と$(T,\mathscr{O}_{T})$を位相空間とし，$p$を
			\begin{align}
				S \times T \ni (s,t) \longmapsto s
			\end{align}
			なる写像とし，$q$を
			...
			なる写像とするとき，
			\begin{align}
				\Set{p^{-1} \ast o}{o \in \mathscr{O}_{S}} \cup
				\Set{q^{-1} \ast o}{o \in \mathscr{O}_{T}}
			\end{align}
			が生成する$S \times T$上の位相を$\mathscr{O}_{S}$と$\mathscr{O}_{T}$の{\bf 積位相}
			\index{せきいそう@積位相}{\bf (product topology)}と呼ぶ．
		\end{dfn}
	\end{screen}
	
	上の定義において$\mathscr{O}_{S}$と$\mathscr{O}_{T}$の積位相を$\mathscr{O}_{S \times T}$とおくと，
	\begin{align}
