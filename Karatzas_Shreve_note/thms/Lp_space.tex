...
\begin{screen}
	\begin{lem}[商空間におけるノルムの定義]
		\begin{align}
			\Norm{[f]}{\mathrm{L}^p} \coloneqq \Norm{f}{\mathscr{L}^p} \quad (f \in \semiLp{p}{X,\mathscr{F},\mu},\ 1 \leq p \leq \infty)
		\end{align}
		により定める$\Norm{\cdot}{\mathrm{L}^p}:\Lp{0}{X,\mathscr{F},\mu} \rightarrow \R$は関数類の代表に依らずに値が確定する．
		そして
		\begin{align}
			\Lp{p}{X,\mathscr{F},\mu} \coloneqq \Set{[f] \in \Lp{0}{X,\mathscr{F},\mu}}{\Norm{[f]}{\mathrm{L}^p} < \infty} \quad (1 \leq p \leq \infty)
		\end{align}
