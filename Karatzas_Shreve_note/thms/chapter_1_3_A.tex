...
\subsection{Fundamental Inequalities}
	...
	\begin{prf}
		...
		\end{align}
		を得る．特に，任意の$m \in \Natural,\ (\lambda > 1/m)$に対して
		\begin{align}
	...
	\begin{itembox}[l]{Lemma: Theorem 3.8 (iii)}
		...
		\begin{align}
			...
			= \sup{n \in \Natural}{U_{F_n}(\alpha,\beta;X)},
			\quad D_{[\sigma,\tau]}(\alpha,\beta;X)
			= \sup{n \in \Natural}{D_{F_n}(\alpha,\beta;X)}.
		\end{align}
	\end{itembox}
	...
	\begin{prf}
		$U_{[\sigma,\tau]}(\alpha,\beta;X) \leq \sup{n \in \Natural}{U_{F_n}(\alpha,\beta;X)}$
		...
		十分大きい$n \in \Natural$に対して或る$t_i,s_i \in F_n,\ (1 \leq i \leq j)$が
		\begin{align}
		...
		\end{align}
		が従い，$\omega$の任意性より$U_F(\alpha,\beta;X) \leq \sup{n \in \Natural}{U_{F_n}(\alpha,\beta;X)}$が出る．
		\QED
	\end{prf}
		...
		\begin{description}
			...
			\item[第二段]
				...
				\begin{align}
					...
					= E \left( \sup{n \in \Natural}{U_{F_n}(\alpha,\beta;X)} \right)
	...
	\begin{itembox}[l]{Theorem 3.8 (iv)}
	...
	\begin{prf}
		...
		$q$を$p$の共役指数として，Fubiniの定理と(\refeq{eq:chapter_1_Theorem_3_8_i_1})及び
		H\Ddot{o}lderの不等式より
		\begin{align}
			\int_{\Omega} Y_n^p\ dP
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第二段]
				...
				\end{align}
				を満たす$\alpha,\beta \in \Q$を取れば，任意の$N \in \Natural$に対し或る点列
				...
				\end{align}
				が従い(\refeq{eq:chapter_1_Theorem_3_8_v_1})が出る．
	...
	\begin{itembox}[l]{Proposition 3.14 (iii)}
		\begin{description}
	...
	\end{itembox}
	\footnotetext{
		$(X_{t+})_{t \geq 0}$が$(\mathscr{F}_{t+})$-適合であることを保証するためには
		$(\mathscr{F}_t)_{t \geq 0}$の完備性が要る．
		また$P$-almostではなく全てのパスがRCLLとなる．
	}
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				(\refeq{eq:chapter_1_Proposition_3_14_1})より$X_{t+}$は可積分である．
