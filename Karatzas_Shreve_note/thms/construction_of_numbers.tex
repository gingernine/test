...
\subsection{整数}
	...
	\monologue{
		院生「前の節で順序数に対して加法と乗法を定義しましたが，加法も乗法も
			...
			しかし二項算法を$\omg$上に制限すれば可換律と簡約律も満たされます．」
	}
	...
	\monologue{
		院生「商``集合''と名前を付けましたが，後で示す通り集合になっています．」
	}
	...
	\begin{screen}
		\begin{thm}[同値類の性質]
			...
			$[y]$で表す．このとき次が成り立つ:
			\begin{description}
				\item[(1)] $\forall y\ \left(\ y \in [y]\ \right)$
				\item[(2)] $\forall y,z \in a\ \left(\ (y,z) \in R \Longleftrightarrow [y] = [z]\ \right)$
				\item[(3)] $\forall y,z \in a\ \left(\ [y] \neq [z] \Longleftrightarrow [y] \cap [z] = \emptyset\ \right)$
			\end{description}
		\end{thm}
	\end{screen}
	...
	\begin{screen}
		\begin{thm}[商写像は全射である]
			$a$を集合とし，$R$を$a$上の同値関係するとき，$a$から$a/R$への商写像は写像であり，さらに言えば全射である．
		\end{thm}
	\end{screen}
	...
	\begin{screen}
		\begin{thm}[商集合の性質]
			...
			$[y]$で表す．このとき次が成り立つ:
			\begin{description}
				\item[(1)] $\forall x\ \left(\ x \in a/R \Longleftrightarrow \exists y \in a\ (\ x=[y]\ )\ \right)$
				\item[(2)] $a/R \in \Univ$
				\item[(3)] ${\displaystyle a = \bigcup (a/R)}$
			\end{description}
		\end{thm}
	\end{screen}
