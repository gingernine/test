...
\subsection{整数}
	\begin{screen}
		\begin{dfn}[商類]
			$a$を類とし，$R$を$a$上の同値関係とする．$x$を$a$の要素とするとき
			\begin{align}
				\Set{y}{(y,x) \in R}
			\end{align}
			を$x$の{\bf 同値類}\index{どうちるい@同値類}{\bf (equivalence class)}と呼び，
			$[x]$などで表す．また$[x]$の要素を$[x]$の{\bf 代表}と呼ぶ．
			また
			\begin{align}
				a / R \coloneqq \Set{x}{\exists y \in a\ \forall z\ (\ (y,z) \in R \Longleftrightarrow z \in x\ )}
			\end{align}
			で定められる類$a/R$を，$a$を$R$で割った{\bf 商類}\index{しょうるい@商類}
			{\bf (quotient class)}と呼び，$a$が集合であるときはそれを
