...
\begin{screen}
	\begin{dfn}[積$\sigma$-加法族]
		...
		\begin{align}
			\mathscr{A} \coloneqq \Set{\bigcap_{\lambda \in \Lambda'} p_\lambda^{-1}(A_\lambda)}{A_\lambda \in \mathcal{M}_\lambda,\ \mbox{$\Lambda'$ is finite subset of $\Lambda$.}}
		\end{align}
		で定める乗法族$\mathscr{A}$が生成する$\sigma$-加法族を
		$\mathcal{M}_\lambda$の積$\sigma$-加法族と呼び，
...
\begin{prf}
	各$S_\lambda$の開集合系及び可算基を$\mathscr{O}_\lambda,\ \mathscr{B}_\lambda$と書く．
\end{prf}
...
\begin{prf}\mbox{}
	\begin{description}
		...
		\item[第二段]
			任意の有限部分集合$T' \subset T$と$B_i \in \borel{S_{\pi(i)}},\ (i \in T')$に対し
			\begin{align}
				\left\{ (X_{\pi(1)}, X_{\pi(2)},\cdots) \in \bigcap_{i \in T'} p_i^{-1}(B_i) \right\}
				= \bigcap_{i \in T'} X_{\pi(i)}^{-1}(B_i)
				\in \sigma(X_i,\ i \in T)
			\end{align}
			が成立するから
			\begin{align}
				\Set{\bigcap_{i \in T'} p_i^{-1}(B_i)}{T' \subset T:finite,\ B_i \in \borel{S_{\pi(i)}}}
