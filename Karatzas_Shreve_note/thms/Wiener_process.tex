\section{Gauss過程}
	...
	\begin{screen}
		\begin{dfn}[座標過程をGauss過程とする測度の一意性]
			$C$を$\mathbf{T}$上の実連続写像の全体とし，$B$を$C$による座標過程とし，
	...
	\begin{prf}
		...
		\end{align}
		なる写像とすれば，$\varphi$は$\R^n$から$\R^n$への同相写像であり
		\begin{align}
			\varphi \ast \left(A_1 \times A_2 \times \cdots \times A_n\right)
			= 
		\end{align}
	\end{prf}
	...
	\begin{screen}
		\begin{dfn}[Wiener測度]
			$C$を$\mathbf{T}$上の実連続写像の全体とし，$B$を$C$による座標過程とし，
			$\mu_w$を$\borel{C}$上の確率測度とする．このとき$B$が$(C,\borel{C},\mu_w)$上のWiener過程であるなら
			$\mu_w$を$\borel{C}$上の{\bf Wiener測度}\index{Wienerそくど@Wiener測度}{\bf (Wiener measure)}と呼ぶ．
		\end{dfn}
	\end{screen}