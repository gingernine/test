\section{The Doob-Meyer Decomposition}
	...
	\begin{itembox}[l]{Remarks 4.6 (i) 修正}
		...
		then $I$ is finite, right-continuous and progressively measurable.}
	\end{itembox}
	...
	\begin{itembox}[l]{Theorem 4.14 修正}
		...
		There exists a continuous version in $[A]_{Nat}$ if and only if $X$ is regular.}
	\end{itembox}
		...
		\begin{description}
			...
			\item[第二段]
				...
				\begin{align}
					...
					\quad (0 \leq t < \infty)
				\end{align}
				で$Z^{(n),j}$は$Y^{(n),j}$の$RCLL$かつ有界な修正となる．
				$A$がナチュラルであるから
				\begin{align}
				...
				\begin{align}
					\xi^{(n)}_t \coloneqq
					\sum_{j=0}^{2^n-1} Z^{(n),j}_t \defunc_{\left(t^{(n)}_j,t^{(n)}_{j+1}\right]},
					\quad (t \geq 0)
				\end{align}
				とおけば
				\begin{align}
