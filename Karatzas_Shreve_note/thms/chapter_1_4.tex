\section{The Doob-Meyer Decomposition}
	...
	\begin{itembox}[l]{Definition 4.5 修正}
		...
		in the meaning of (\refeq{eq:equivalence_with_respect_to_path}) as $[A]_{Nat}\ ( \subset Nat[0,\infty))$.}
	\end{itembox}
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第二段]
				$X$がレギュラーであるとし，任意に$a > 0$を取り
				\begin{align}
					t^{(n)}_j \coloneqq \frac{j}{2^n}a,\quad (j=0,1,\cdots,2^n)
				\end{align}
				とおく．任意に$\lambda > 0$を取り，各$j = 0,1,\cdots,2^n$に対し
				\begin{align}
					Y^{(n),j}_t \coloneqq
					\begin{cases}
						\lambda \wedge A_{t^{(n)}_{j+1}}, & (t > t^{(n)}_{j+1}), \\
						\cexp{\lambda \wedge A_{t^{(n)}_{j+1}}}{\mathscr{F}_t}, & (0 \leq t \leq t^{(n)}_{j+1})
					\end{cases}
				\end{align}
				とおけば$\Set{Y^{(n),j}_t,\mathscr{F}_t}{0 \leq t < \infty}$はマルチンゲールとなり
				\begin{align}
					[0,\infty) \ni t \longmapsto EY^{(n),j}_t 
					= E\left(\lambda \wedge A_{t^{(n)}_{j+1}}\right)
				\end{align}
				より$RCLL$な修正$\tilde{Y}^{(n),j}$が存在する(Theorem 3.13)．またこのとき
				\begin{align}
				...
				\end{align}
				より全ての$t \geq 0$で$P$-a.s.に$\tilde{Y}^{(n),j}_t \leq \lambda$となるから
				\begin{align}
					E \coloneqq \Set{\tilde{Y}^{(n),j}_t > \lambda}{\exists t \geq 0}
					= \bigcup_{r \in [0,\infty)\cap\Q}\left\{\tilde{Y}^{(n),j}_r > \lambda\right\}
				\end{align}
				により$P$-零集合を定まり，usual条件より$E \in \mathscr{F}_0$であるから
				\begin{align}
					Z^{(n),j}_t \coloneqq \tilde{Y}^{(n),j}_t \defunc_{\Omega \backslash E},
					\quad (0 \leq t < \infty)
				\end{align}
				で$Z^{(n),j}$は$Y^{(n),j}$の$RCLL$かつ有界な修正マルチンゲールとなる．
				$A$がナチュラルであるから
				\begin{align}
					\int_{\left(t^{(n)}_j,t^{(n)}_{j+1}\right]} Z^{(n),j}_s\ dA_s
					&= \int_{\left(0,t^{(n)}_{j+1}\right]} Z^{(n),j}_s\ dA_s
						- \int_{\left(0,t^{(n)}_j\right]} Z^{(n),j}_s\ dA_s \\
					&= \int_{\left(0,t^{(n)}_{j+1}\right]} Z^{(n),j}_{s-}\ dA_s
						- \int_{\left(0,t^{(n)}_j\right]} Z^{(n),j}_{s-}\ dA_s \\
					&= \int_{\left(t^{(n)}_j,t^{(n)}_{j+1}\right]} Z^{(n),j}_{s-}\ dA_s
				\end{align}
				となる．
				\begin{align}
					\xi^{(n)}_t \coloneqq
				...
				とおけば
				\begin{align}
					\int_{(0,a]} \xi^{(n)}_s\ dA_s
					= \sum_{j=0}^{2^n-1} \int_{\left(t^{(n)}_j,t^{(n)}_{j+1}\right]} Z^{(n),j}_s\ dA_s
					= \sum_{j=0}^{2^n-1} \int_{\left(t^{(n)}_j,t^{(n)}_{j+1}\right]} Z^{(n),j}_{s-}\ dA_s
					= \int_{(0,a]} \xi^{(n)}_{s-}\ dA_s
				\end{align}
				が成立する．同様に
				\begin{align}
					\eta^{(n)}_t \coloneqq
				...
				\end{align}
				とおけば$\eta^{(n)}$は右連続かつ
				$t \neq t^{(n)}_j,\ (j=0,1,\cdots,2^n)$なら
				$\xi^{(n)}_t = \eta^{(n)}_t$を満たし，また
				\begin{align}
					\int_A \eta^{(n)}_t\ dP
					= \int_A \lambda \wedge A_{t^{(n)}_{j+1}}\ dP
					\geq \int_A \lambda \wedge A_{t^{(n+1)}_{}}\ dP
					= \int_A \eta^{(n+1)}_t\ dP,
					\quad (\forall A \in \mathscr{F}_t,\ t^{(n)}_j \leq t < t^{(n)}_{j+1})
				\end{align}
				より$\eta^{(n)}_t \geq \eta^{(n+1)}_t\ \mbox{a.s. $P$}$となる．
				$\eta^{(n)}$の右連続性より
				\begin{align}
					F_n \coloneqq \Set{\eta^{(n)}_t < \eta^{(n+1)}_t}{\exists t \geq 0}
					= \bigcup_{r \in [0,\infty) \cap \Q} \left\{\eta^{(n)}_r < \eta^{(n+1)}_r\right\}
				\end{align}
				で$P$-零集合が定まり，$F \coloneqq \bigcup_{n=1}^\infty E_n$及び
				\begin{align}
				...
				とおけば
				\begin{align}
					T^{(1)}_\epsilon(\omega) \geq T^{(2)}_\epsilon(\omega) \geq \cdots,
					\quad (\forall \omega \in \Omega \backslash F)
				\end{align}
				Corollary 2.4 と Problem2.5 及びusual条件より$T^{(n)}_\epsilon$は
				$(\mathscr{F}_t)$-停止時刻であり
				\begin{align}
				...
				\end{align}
				より$T^{(n)}_\epsilon \defunc_{\Omega \backslash F}$も停止時刻，
				\begin{align}
					T_\epsilon \coloneqq \lim_{n \to \infty} T^{(n)}_\epsilon \defunc_{\Omega \backslash F}
				\end{align}
				とおけば
				\begin{align}
					\left\{T_\epsilon < t \right\} = \bigcup_{n=1}^\infty
					\left\{ T^{(n)}_\epsilon \defunc_{\Omega \backslash F} < t \right\}
					\in \mathscr{F}_t,\quad (\forall t \geq 0)
				\end{align}
				と$(\mathscr{F}_t)$の右連続性より$T_\epsilon \in \mathscr{S}_a$となる．
		\end{description}
	\end{prf}