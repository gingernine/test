		...
		\begin{description}
			...
			\item[第二段]
				...
				\begin{align}
					\int_A \tilde{Y}^{(n),j}_t\ dP
					= \int_A \lambda \wedge A_{t^{(n)}_j}\ dP
					\leq \int_A \lambda \wedge A_t\ dP,
					\quad (\forall A \in \mathscr{F}_t) 
				\end{align}
				となるから
				\begin{align}
					E_j &\coloneqq \Set{\tilde{Y}^{(n),j}_t > \lambda}{\exists t \geq 0} 
						\cup \Set{\lambda \wedge A_t < \tilde{Y}^{(n),j}_t}{\exists t \in \left[t^{(n)}_j, t^{(n)}_{j+1} \right)} \\
					&= \left[ \bigcup_{r \in [0,\infty)\cap\Q}\left\{\tilde{Y}^{(n),j}_r > \lambda\right\} \right]
					\bigcup \left[ \bigcup_{r \in \left[t^{(n)}_j, t^{(n)}_{j+1} \right)\cap\Q}\left\{\lambda \wedge A_r < \tilde{Y}^{(n),j}_r\right\} \right], \\
				...
				\begin{align}
					...
					\begin{cases}
						...
						Z^{(n),j}_t(\omega) \leq \lambda \wedge A_t(\omega), & \forall t \in \left[t^{(n)}_j, t^{(n)}_{j+1} \right)
					\end{cases}
				\end{align}
				...
				\begin{align}
					...
					\begin{cases}
						...
						\eta^{(n)}_t(\omega) \leq \lambda \wedge A_t(\omega), & \forall t \in [0,a]
					\end{cases}
				...
				\begin{align}
					...
					a \wedge \inf{}{\Set{t \geq 0}{(\lambda \wedge A_t) - \eta^{(n)}_t > \epsilon}}
				\end{align}
				...
				\end{align}
				が得られる．また$(\lambda \wedge A) - \eta^{(n)}$の右連続性より
				\begin{align}
					T^{(n)}_\epsilon(\omega) < a
					\quad \Longrightarrow
					\quad \left(\lambda \wedge A_{T^{(n)}_\epsilon}(\omega)\right) - \eta^{(n)}_{T^{(n)}_\epsilon}(\omega)
						\geq \epsilon
				\end{align}
				となるから
				\begin{align}
					E\left(\lambda \wedge A_{T^{(n)}_\epsilon}
						- \lambda \wedge A_{\varphi_n\left(T^{(n)}_\epsilon\right)} \right)
					= E\left(\lambda \wedge A_{T^{(n)}_\epsilon}
						- \eta^{(n)}_{T^{(n)}_\epsilon} \right)
					= E\defunc_{\left\{T^{(n)}_\epsilon < a\right\}}\left(\lambda \wedge A_{T^{(n)}_\epsilon}
						- \eta^{(n)}_{T^{(n)}_\epsilon} \right)
			...
			\item[第三段]
				...
				$P$-a.s. に増大していく．実際，任意の$t \in [0,a)$に対し
				\begin{align}
				...
				\begin{align}
					...
					= \int_A \lambda \wedge A_{t^{(n)}_j}\ dP
					\begin{cases}
						\displaystyle= \int_A \lambda \wedge A_{t^{(n+1)}_{2j}}\ dP \\
						\displaystyle\leq \int_A \lambda \wedge A_{t^{(n+1)}_{2j+1}}\ dP
					\end{cases}
					= \int_A \eta^{(n+1)}_t\ dP
				\end{align}
				が成り立ち$\eta^{(n)}_t \leq \eta^{(n+1)}_t,\ \mbox{a.s. $P$}$が従う．
				...
				$\left( \eta^{(n)}_t(\omega) \right)_{n=1}^\infty$は増大し
				\begin{align}
				...
				も$\mathscr{S}_a$の列で，$\Omega \backslash F$の上で単調に増大し
				\begin{align}
				...
				が従い，また$(\varphi_n)_n$の増大性と$t \longmapsto \varphi_n(t)$の増大性より
				\begin{align}
				...
				\end{align}
				が成り立つ．任意に$\delta > 0$と$\omega \in \Omega \backslash F$を取れば
				\begin{align}
			...
			\item[第五段]
				...
				$\Omega \backslash E$の上で$(\lambda \wedge A_t) - \eta^{(n)}_t \geq 0,\ (\forall t \in [0,a])$であるから，
				(\refeq{eq:chapter_1_theorem_4_14_1}), (\refeq{eq:chapter_1_theorem_4_14_5})と併せて
				\begin{align}
					&P\left(\sup{0 \leq t \leq a}{\left|(\lambda \wedge A_t) - \eta^{(n)}_t\right|} > \epsilon\right)
					= P\left(T^{(n)}_\epsilon < a\right) \\
					&\qquad \leq \frac{1}{\epsilon} E\left(\lambda \wedge A_{T^{(n)}_\epsilon}
						- \lambda \wedge A_{\varphi_n\left(T^{(n)}_\epsilon\right)} \right)
				...
				\begin{align}
					\sup{0 \leq t \leq a}{\left(\lambda \wedge A_t(\omega))-|\eta^{(n_k)}_t(\omega)\right|}
					\longrightarrow 0
					\quad (k \longrightarrow \infty),
