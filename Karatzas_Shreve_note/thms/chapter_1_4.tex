	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第三段]
				...
				%}
				上で分解の存在を示す．$\Set{Y_t,\mathscr{F}_t}{0 \leq t \leq a}$を
				\begin{align}
					Y_t \coloneqq X_t - \cexp{X_a}{\mathscr{F}_t},
					\quad (t \in [0,a])
				\end{align}
				で定めれば劣マルチンゲールとなり，
				$X$の劣マルチンゲール性より任意の$t \in [0,a]$で
				\begin{align}
					Y_t \leq 0, \quad \mbox{a.s. $P$}
					\label{eq:chapter_1_theorem_4_10_4}
				\end{align}
				となる．
				\begin{align}
			...
			\item[第四段]
				...
				\begin{align}
					Y_T = X_T - \cexp{X_a}{\mathscr{F}_T},\quad \mbox{a.s. $P$}
					\label{eq:chapter_1_theorem_4_10_2}
				\end{align}
				が成立する．実際$M_t \coloneqq \cexp{X_a}{\mathscr{F}_t}$で
				マルチンゲール$\Set{M_t,\mathscr{F}_t}{0 \leq t < \infty}$を定めれば，
				任意抽出定理より
				\begin{align}
					\int_A M_T\ dP = \int_A \cexp{M_a}{\mathscr{F}_T}\ dP
					= \int_A \cexp{X_a}{\mathscr{F}_T}\ dP,
					\quad (\forall A \in \mathscr{F}_T)
				\end{align}
				が成立し，$M_T = X_T - Y_T$であるから(\refeq{eq:chapter_1_theorem_4_10_2})が得られる．
				$X$はクラス$DL$に属し，$\left(\cexp{X_a}{\mathscr{F}_T}\right)_{T \in \mathscr{S}_a}$は
				定理\ref{lem:uniformly_integrability_and_conditional_expectations}より一様可積分であるから，
				$(Y_T)_{T \in \mathscr{S}_a}$の一様可積分性が従う．
			...
			\item[第五段]
				...
				\begin{align}
					...
					&= \sum_{j=1}^{2^n} \int_{\Lambda \cap \left\{T^{(n)}_\lambda=t^{(n)}_{j-1}\right\}} 
						A^{(n)}_{t^{(n)}_{j-1}} - A^{(n)}_a\ dP \\
					&= \sum_{j=1}^{2^n} \int_{\Lambda \cap \left\{T^{(n)}_\lambda=t^{(n)}_{j-1}\right\}} 
						A^{(n)}_{T^{(n)}_\lambda} - \cexp{A^{(n)}_a}{\mathscr{F}_{T^{(n)}_\lambda}}\ dP \\
					&= \int_\Lambda A^{(n)}_{T^{(n)}_\lambda} - \cexp{A^{(n)}_a}{\mathscr{F}_{T^{(n)}_\lambda}}\ dP
				\end{align}
				が成立し
				\begin{align}
					Y_{T^{(n)}_\lambda} = A^{(n)}_{T^{(n)}_\lambda} - \cexp{A^{(n)}_a}{\mathscr{F}_{T^{(n)}_\lambda}},
					\quad \mbox{a.s. $P$}
					\label{eq:chapter_1_theorem_4_10_5}
				\end{align}
				が得られるから，(\refeq{eq:chapter_1_theorem_4_10_3})と(\refeq{eq:chapter_1_theorem_4_10_1})より
				\begin{align}
					\int_{\left\{A^{(n)}_a > \lambda\right\}} A^{(n)}_a\ dP
					= \int_{\left\{T^{(n)}_\lambda < a\right\}} \cexp{A^{(n)}_a}{\mathscr{F}_{T^{(n)}_\lambda}}\ dP
					\leq \lambda P\left( T^{(n)}_\lambda < a\right) 
						- \int_{\left\{T^{(n)}_\lambda < a\right\}} Y_{T^{(n)}_\lambda}\ dP
				\end{align}
				となる．一方で(\refeq{eq:chapter_1_theorem_4_10_4})，(\refeq{eq:chapter_1_theorem_4_10_6})，
				(\refeq{eq:chapter_1_theorem_4_10_3})，(\refeq{eq:chapter_1_theorem_4_10_1})，
				(\refeq{eq:chapter_1_theorem_4_10_5})より
				\begin{align}
					\int_{\left\{T^{(n)}_{\lambda/2} < a\right\}} Y_{T^{(n)}_{\lambda/2}}\ dP
					\leq \int_{\left\{T^{(n)}_{\lambda} < a\right\}} Y_{T^{(n)}_{\lambda/2}}\ dP
					&= \int_{\left\{T^{(n)}_{\lambda} < a\right\}} A^{(n)}_{T^{(n)}_{\lambda/2}} - A^{(n)}_a\ dP \\
					&= \int_{\left\{A^{(n)}_a > \lambda\right\}} A^{(n)}_{T^{(n)}_{\lambda/2}} - A^{(n)}_a\ dP \\
					&\leq -\frac{\lambda}{2} P\left(A^{(n)}_a > \lambda\right)
					= -\frac{\lambda}{2} P\left(T^{(n)}_{\lambda} < a\right)
				\end{align}
				が成立するから
				\begin{align}
					\int_{\left\{A^{(n)}_a > \lambda\right\}} A^{(n)}_a\ dP
				...
				\begin{align}
					...
					\leq \sup{n \in \N}{\int_{\left\{T^{(n)}_{\lambda/2} < a\right\}} \left|Y_{T^{(n)}_{\lambda/2}}\right|\ dP}
			...
			\item[第七段]
				...
				\end{align}
				が任意の$k \geq 1$で成り立ち，$k \longrightarrow \infty$として
				\begin{align}
