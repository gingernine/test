	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第四段]
				...
				\end{align}
				となるから$T_\lambda^{(n)}$は$(\mathscr{F}_t)$-停止時刻であり$T^{(n)}_\lambda \in \mathscr{S}_a$を満たす．また
				\begin{align}
				...
				\end{align}
				となる．任意の$\Lambda \in \mathscr{F}_{T^{(n)}_\lambda}$に対し
				$\Lambda \cap \left\{T^{(n)}_\lambda=t^{(n)}_{j-1}\right\} \in \mathscr{F}_{t^{(n)}_{j-1}}$となるから
				\begin{align}
				...
				\begin{align}
					...
					\leq \lambda - \cexp{A^{(n)}_a}{\mathscr{F}_{T^{(n)}_\lambda}},
					\quad \mbox{a.s. $P$}
				\end{align}
				が得られるから(\refeq{eq:chapter_1_theorem_4_10_1})より
				\begin{align}
