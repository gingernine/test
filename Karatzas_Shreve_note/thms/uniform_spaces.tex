\subsection{一様空間}
	...
	\begin{screen}
		\begin{thm}[一様位相空間において$T_0 \Longleftrightarrow T_2 \Longleftrightarrow T_{3{}^1{\mskip -5mu/\mskip -3mu}_2}$]
		\label{thm:T_0_iff_T_2_on_uniform_topological_space}
			...
			\begin{align}
				...
				\mbox{$S$が$T_2$}
				\quad \Longleftrightarrow \quad
				\mbox{$S$が$T_{3{}^1{\mskip -5mu/\mskip -3mu}_2}$}.
	...
	\begin{prf}
		...
		\end{align}
		となるから$\bigcap \mathscr{V} = \Set{(x,x)}{x \in S}$が従い，
		またこのとき相異なる任意の二点は近傍で分離されるから$S$はHausdorffとなる．
		一様空間は完全正則であるから，$S$が$T_0$なら$S$は$T_{3{}^1{\mskip -5mu/\mskip -3mu}_2}$となり，
		$S$が$T_{3{}^1{\mskip -5mu/\mskip -3mu}_2}$なら$T_0$であるから
		(\refeq{eq:thm_T_0_iff_T_2_on_uniform_topological_space})を得る．
		\QED
	\end{prf}
