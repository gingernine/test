\subsection{一様空間}
	...
	\begin{screen}
		\begin{thm}[近縁系で導入する位相]\label{thm:topology_induced_by_the_uniformity}
			...
			の(LB1)(LB2)(LB3)を満たす．このとき$\{\mathscr{U}(x)\}_{x \in S}$を基本近傍系とする
			$S$の位相がただ一つ定まるが，別の基本近縁系を用いても同じ位相が定まる．
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		...
		$\{\mathscr{U}(x)\}_{x \in S}$を基本近傍系とする$S$の位相
		$\tau_{\mathscr{U}}$がただ一つ定まる．
		$\tilde{\mathscr{U}}$を$\mathscr{V}$の別の基本近縁系として
		\begin{align}
			\tilde{\mathscr{U}}(x) \coloneqq \Set{\tilde{U}_x}{\tilde{U} \in \tilde{\mathscr{U}}},
			\quad (\forall x \in S)
		\end{align}
		とおくとき，$\left\{\tilde{\mathscr{U}}(x)\right\}_{x \in S}$は
		$(S,\tau_{\mathscr{U}})$における基本近傍系となる．
		実際任意の$\tilde{U} \in \tilde{\mathscr{U}}$に対し
		$U \subset \tilde{U}$を満たす$U \in \mathscr{U}$が存在して
		$U_x \subset \tilde{U}_x$となるから$\tilde{U}_x$は$x$の近傍であり，
		任意の$U_x \in \mathscr{U}(x)$に対し或る$\tilde{V} \in \tilde{\mathscr{U}}$で
		$\tilde{V} \subset U$となり$\tilde{V}_x \subset U_x$が満たされるから
		$\tilde{\mathscr{U}}(x)$は$x$の基本近傍系となる．
		定理\ref{thm:local_base_defines_open_sets}より
		$\left\{\tilde{\mathscr{U}}(x)\right\}_{x \in S}$を基本近傍系とする位相
		$\tau_{\tilde{\mathscr{U}}}$は$\tau_{\mathscr{U}}$に一致する．
		\QED
	\end{prf}
	...
	\begin{screen}
		\begin{thm}[部分一様空間]
			...
			\end{align}
			は$A$上の近縁系となる．また$S$と$A$にそれぞれ$\mathscr{V},\ \mathscr{V}_A$で
			一様位相$\tau,\ \tau_A$を導入すれば次の関係が満たされる:
			\begin{align}
				\tau_A = \Set{O \cap A}{O \in \tau}.
			\end{align}
		\end{thm}
	\end{screen}
