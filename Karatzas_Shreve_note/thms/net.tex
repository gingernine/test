\section{有向点族}
	...
	\begin{screen}
		...
		\end{dfn}
	\end{screen}
	正の自然数全体$\Natural$や実数全体$\R$は，通常の順序により
	...
	\begin{screen}
		...
		\end{dfn}
	\end{screen}
	特に$\Natural$を有向集合とする有向点族を{\bf 点列}\index{てんれつ@点列}{\bf (sequence)}と呼ぶ．
	また点列$(x_n)_{n \in \Natural}$に対し
	\begin{align}
		f:\Natural \ni k \longmapsto n_k \in \Natural,
		\quad (n_1 < n_2 < n_3 < \cdots)
	\end{align}
	で定まる部分有向点族$\left(x_{n_k}\right)_{k \in \Natural}$
	...
	\begin{screen}
		\begin{thm}[Hausdorff$\Longleftrightarrow$収束する有向点族の極限は一つ]
	...
	\begin{prf}
		...
		$\{U_n\}_{n \in \Natural},\ \{V_n\}_{n \in \Natural}$をそれぞれ
		$x,y$の可算な基本近傍系として
		\begin{align}
			\tilde{U}_n \coloneqq U_1 \cap U_2 \cap \cdots \cap U_n,
			\quad \tilde{V}_n \coloneqq V_1 \cap V_2 \cap \cdots \cap V_n,
			\quad (\forall n \in \Natural)
		\end{align}
		で単調減少な基本近傍系$\left\{\tilde{U}_n\right\}_{n \in \Natural}$と
		$\left\{\tilde{V}_n\right\}_{n \in \Natural}$を定め
		$w \in \prod_{n \in \Natural} \left(\tilde{U}_n \cap \tilde{V}_n\right)$を取れば，
		点列$w = (w_n)_{n \in N}$は$x,y$の両方に収束する．
		従って(\refeq{eq:thm_Hausdorff_iff_at_most_one_limit_1})の
		$\Longleftarrow$も得られる．
		\QED
	\end{prf}
	...
	\begin{prf}
				...
				任意の$n \in \Natural$に対して
				\begin{align}
					\inprod<n> \coloneqq
					\Set{m \in \Natural}{n < m,\ x_m \notin U}
				\end{align}
				は空ではない．
				$\Natural$の空でない部分集合の全体を$\mathscr{N}$として
				...
				\begin{align}
					...
					&\vdots
				\end{align}
				で$\{n_k\}_{k \in \Natural}$を定めれば，
				$(x_{n_k})_{k \in \Natural}$は$a$に収束しない部分列となる．
				\QED
	\end{prf}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段] $\Longrightarrow$を示す．
				...
				つまり$\mathscr{U} = \{U_n\}_{n \in \Natural}$と表せるとき，
				\begin{align}
					\tilde{U}_n \coloneqq U_1 \cap U_2 \cap \cdots \cap U_n,
					\quad (n=1,2,\cdots) 
				\end{align}
				で減少列$\tilde{\mathscr{U}} \coloneqq \{\tilde{U}_n\}_{n \in \Natural}$を定めれば
				$\tilde{\mathscr{U}}$も$a$の基本近傍系となり，
				$y \in \prod_{n \in \Natural}(A \cap \tilde{U}_n)$を取り
				\begin{align}
					y_n \coloneqq y(n),\quad (\forall n \in \Natural)
				\end{align}
				とおけば，$a$の任意の近傍$V$に対し$\tilde{U}_{n_0} \subset V$
				となる$n_0 \in \Natural$が存在して
				\begin{align}
	...
	\begin{screen}
		\begin{thm}[$f$が$x$で連続$\Longleftrightarrow$
		$x$に収束する有向点族の像点族が$f(x)$に収束]\label{thm:continuity_and_net}
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[(2)] 点列は有向点族であるから(1)より必要性が従う．
				$\mathscr{U} = \{U_n\}_{n \in \Natural}$を$s$の基本近傍系すれば，
				\begin{align}
					\tilde{U}_n \coloneqq
					U_1 \cap U_2 \cap \cdots \cap U_n,
					\quad (n=1,2,\cdots)
				\end{align}
				により単調減少な$s$の基本近傍系$\left\{\tilde{U}_n\right\}_{n \in \Natural}$
				...
				\begin{align}
					...
					\quad (\forall n \in \Natural)
				\end{align}
				が成り立ち，$x \in \prod_{n \in \Natural} \tilde{U}_n \backslash f^{-1}(V)$
				を取れば$x = (x_n)_{n \in \Natural}$は
	...
	\begin{screen}
		...
		\label{thm:a_net_frequently_in_all_nbhs_iff_some_subnet_converges}\mbox{}
			\begin{description}
				...
				\item[(2)]
					(1)において$\Lambda = \Natural$かつ$a$が可算な基本近傍系を持つ場合，
					\begin{align}
						\mbox{$(x_n)_{n \in \Natural}$が$a$の任意の近傍に無限に含まれる}
						\quad \Longleftrightarrow \quad
						\mbox{$a$に収束する$(x_n)_{n \in \Natural}$の部分列が存在する}.
					\end{align}
			\end{description}
		\end{thm}
	\end{screen}
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第二段] (2)の$\Longrightarrow$を示す．
				$\mathscr{U} = \{U_k\}_{k \in \Natural}$を$a$の基本近傍系とすれば
				\begin{align}
					V_k \coloneqq U_1 \cap U_2 \cap \cdots \cap U_k,
					\quad (k=1,2,\cdots)
				\end{align}
				により単調減少な基本近傍系$\{V_k\}_{k \in \Natural}$が得られる．
				任意の$n,k \in \Natural$に対し
				\begin{align}
					\inprod<n,k> \coloneqq
					\Set{m \in \Natural}{n < m,\ x_m \in V_k}
				\end{align}
				とおけば$\inprod<n,k>$は空ではなく，
				選択関数$\Phi \in \prod_{n,k \in \Natural} \inprod<n,k>$を取り
				\begin{align}
					...
					&\vdots
				\end{align}
				で$\{n_k\}_{k \in \Natural}$を定めれば，$(x_{n_k})_{k \in \Natural}$は
				$(x_n)_{n \in \Natural}$の部分列となり，任意の$V_{k_0}$に対して
				\begin{align}
					k_0 \leq k \quad \Longrightarrow \quad
					x_{n_k} \in V_k \subset V_{k_0}
				\end{align}
				となるから$(x_{n_k})_{k \in \Natural}$は$a$に収束する．
