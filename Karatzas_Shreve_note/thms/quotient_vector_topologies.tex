	...
	\begin{sketch}\mbox{}
		\begin{description}
			...
			\item[第二段]
				...
				とおく．このとき
				\begin{align}
					\mathscr{B}_q \defeq \Set{u}{\exists b \in \mathscr{B}\, (\, u = q \ast b\, )}
				\end{align}
			...
			\item[第三段]
				$\sigma_q$が$(0_q,0_q)$において$\mathscr{O}_q$に関して連続であることを示す．
				$u$を$0_q$の近傍とすると
				\begin{align}
					q \ast b \subset u
				\end{align}
			...
			\item[第四段] 
				まず$\mathscr{B}_q$の要素が均衡集合であることを示す．$u$を$\mathscr{B}_q$の要素とすると
				\begin{align}
					u = q \ast b
				\end{align}
				を満たす$\mathscr{B}$の要素$b$が取れる．ここで$\alpha$を
				\begin{align}
					|\alpha| \leq 1
				\end{align}
				なる$\Phi$の要素とし，$x$を$u$の任意の要素とすれば，
				\begin{align}
					x = q(\eta)
				\end{align}
				を満たす$b$の要素$\eta$が取れて，$b$は均衡しているので
				\begin{align}
					s(\alpha,\eta) \in b 
				\end{align}
				が成り立つ．よって
				\begin{align}
					s_q(\alpha,x) = q\left(s(\alpha,\eta)\right) \in q \ast b
				\end{align}
				が成り立つ．ゆえに$u$は均衡集合である．よっていま
				\begin{align}
				...
				$\mathscr{V}$は$X_q$上の近縁系をなす．この$\mathscr{V}$で導入する$X$上の一様位相が
				定理\ref{thm:entourages_introducing_vector_topology}の四条件を満たすことを確認する．
				
				
			\item[第五段]
				$\mathscr{O}_q$と$\mathscr{V}$で導入する$X$上の一様位相が一致することを示す．
