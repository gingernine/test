\subsection{順序数}
	...
	\begin{itemize}
		\item $a \subset \ON \wedge a \neq \emptyset 
			\Longrightarrow \exists x \in a\ \forall y \in a\ (\ x \subset y\ )$.
		
		\item $\forall a\ (\ a \in \ON \Longrightarrow a \subset \ON\ )$
		
		\item $\forall a\ (\ a \subset \ON \Longrightarrow \bigcup a \in \ON\ )$
		
		\item $\forall a,b \in \ON\ (\ a \in b \Longleftrightarrow a \subsetneq b\ )$
		
		\item $R = \Set{x}{\exists a,b \in \ON\ (\ x=(a,b) \wedge a \subset b\ )}$は全順序
