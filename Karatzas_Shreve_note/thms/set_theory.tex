	...
	\begin{prf}
		...
		\end{align}
		が得られる．ゆえに外延性の公理より$a = \emptyset$が成立し，演繹法則より定理の主張が従う．
		\QED
	\end{prf}
	...
	\begin{prf}
		...
		\begin{align}
			a \in b \Longrightarrow \set{a}
		\end{align}
		が成立する．このとき$\tau \coloneqq \varepsilon x (\ a = x\ )$とおけば$a = \tau$が成り立つので
		相等性の公理より$\tau \in b$が成立し，推論規則\ref{metaaxm:rules_of_quantifiers}より
		$\exists x\ (\ x \in b\ )$が成り立つ．よって演繹法則より
		\begin{align}
			a \in b \Longrightarrow \exists x\ (\ x \in b\ )
		\end{align}
		が成り立つから，対偶を取り推論法則\ref{metathm:properties_of_quantifiers}を適用すれば
		\begin{align}
			\forall x\ (\ x \notin b\ ) \Longrightarrow a \notin b
		\end{align}
		が成り立つ．$b$を$\emptyset$に置き換えれば$\forall x\ (\ x \notin \emptyset\ )$が満たされるので
		三段論法より$a \notin \emptyset$が出る．
		\QED
	\end{prf}
	...
	\monologue{
		院生「我々は$\mathcal{L}$の式$A$を用いて$\Set{x}{A(x)}$の記法を導入しましたが，
			...
			同然ながらそれは$B$が同値な$\mathcal{L}$の式で表現できることが条件です．
			ここで真価を発揮するのは$\varepsilon$記号です．後述する通り，
	...
	\end{itemize}
	として記号列$\varepsilon a(x)$を定める．$\varepsilon$記号を用いているのは，
	量化記号に関する推論規則で$\varepsilon$記号を定めたときと動機が似ているためである．
	早速この記法を用いて次を定義する．
	...
	\begin{align}
		\Set{x}{x = a \vee x = b}
	\end{align}
	という表しても良いことにする．もとより，対の定義はこちらの表記を正当化することを予期したものである．
	...
	\begin{screen}
		\begin{thm}[真類の対は空・集合は自分を構成要素とする対に属する]
			$a,b$を類とするとき次が成り立つ:
			\begin{itemize}
				\item $\rightharpoondown \set{a} \wedge \rightharpoondown \set{b} \Longrightarrow \{a,b\} = \emptyset.$
				\item $\set{a} \Longrightarrow a \in \{a,b\}.$
			\end{itemize}
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		...
		が成立する．他方で
		\begin{align}
			\forall x\ \left(\ a = x \vee b = x \Longleftrightarrow x \in \{a,b\}\ \right)
		\end{align}
		が満たされているので，ここで$\chi$を$\mathcal{L}$の任意の対象とすれば
		$\wedge$の除去と対偶命題の同値性から
		\begin{align}
			a \neq \chi \vee b \neq \chi \Longrightarrow \chi \notin \{a,b\}
		\end{align}
		が成り立ち，$a \neq \chi \wedge b \neq \chi$も満たされているので三段論法より$\chi \notin \{a,b\}$が成立する．
		そして$\chi$の任意性と推論法則\ref{metathm:fundamental_law_of_universal_quantifier}より
		\begin{align}
			\forall x\ (\ x \notin \{a,b\}\ )
		\end{align}
		が成立し，定理\ref{thm:uniqueness_of_emptyset}より$\{a,b\} = \emptyset$が従うので，演繹法則を適用して
		\begin{align}
			\rightharpoondown \set{a} \wedge \rightharpoondown \set{b} \Longrightarrow \{a,b\} = \emptyset
		\end{align}
		が得られる．次に$\set{a}$が成立していると仮定する．このとき存在記号に関する規則より
		\begin{align}
			a = \varepsilon x(\ a = x\ ) \Longleftrightarrow \set{a}
		\end{align}
		が成り立つので，$\tau \coloneqq \varepsilon x(\ a = x\ )$とおけば
		三段論法より$\tau = a$が成立し，$\vee$の導入より$\tau = a \vee \tau = b$が成り立ち
		\begin{align}
			\tau \in \{a,b\}
		\end{align}
		となる．そして相等性の公理より$a \in \{a,b\}$が従う．ここで演繹法則を適用すれば
		\begin{align}
	...
	\end{align}
	が成立する．実際，$\varepsilon$記号の定め方と類の公理から
	\begin{align}
	...
	\begin{align}
		...
		\Longleftrightarrow \exists t\ (\ t \in a \wedge x \in t\ )\ \right)
	\end{align}
	が成立する．これを根拠にして，$\bigcup a$を
	\begin{align}
		\Set{x}{\exists t \in a\ (\ x \in t\ )}
	\end{align}
	で表しても良いことにする．正確さも大切であるが，やはり判りやすい方が良いに決まっている．
	...
	\begin{screen}
		\begin{thm}[合併の性質]\mbox{}
			\begin{description}
				...
				\item[(ii)] $\forall x\ (\ x \in a \cup b \Longleftrightarrow x \in a \vee x \in b\ )$
