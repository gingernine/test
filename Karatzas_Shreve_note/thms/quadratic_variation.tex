\section{二次変分}
	...
	\begin{screen}
		...
		\label{thm:decomposition_of_square_integrable_martingales}
			...
			\end{align}
			を満たす連続な$\{\mathscr{F}_t\}_{t \in \mathbf{T}}$-増大過程$A$が取れる．
		\end{thm}
	\end{screen}
	...
	\begin{sketch}
		\begin{description}
			...
			\item[step1-2] $n$を自然数とし，
				...
				\end{align}
				となる．以上で(\refeq{fom:thm_decomposition_of_square_integrable_martingales_2})が得られた．
	...
	\begin{screen}
		\begin{thm}[二次変分の存在]
			$X$を$\mathscr{M}_{c,loc}$の要素とすると，
			\begin{align}
				X^{2} - A \in \mathscr{M}_{c,loc}
			\end{align}
			を満たす連続な$\{\mathscr{F}_{t}\}_{t \in [0,1]}$-増大過程$A$が取れる．
		\end{thm}
	\end{screen}
	...
	\begin{screen}
		\begin{thm}[]
			\begin{align}
				X \in \mathscr{M}_{c}^{2} \Longleftrightarrow E\inprod<M>_{1} < \infty.
			\end{align}
		\end{thm}
	\end{screen}
