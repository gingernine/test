\section{二乗可積分マルチンゲール}
	...
	\begin{screen}
		\begin{thm}[$\mathscr{M}^2_{\mathbf{T}}$は完備な擬距離空間]\label{thm:pseudo_metric_on_square_integrable_martingales}
	...
	\begin{sketch}\mbox{}
		\begin{description}
			...
			\item[step1-2] $n$を自然数とし，
				...
				\end{align}
				であることを示す．まず$X$の右連続性より$A^{(n)}$も右連続である．また$X$が連続である場合は$A^{(n)}$も連続である．
				全ての$j$で
				\begin{align}
				...
				\begin{align}
					...
					= \cexp{ X^2_{\min\left\{t,\frac{j+1}{2^n}T\right\}} - X^2_{\min\left\{t,\frac{j}{2^n}T\right\}}}{\mathscr{F}_s}
				\end{align}
