\section{二乗可積分マルチンゲール}
	...
	\begin{screen}
		\begin{thm}[右連続マルチンゲールは局所的に有界にできる]
			$\mathbf{T} = [0,T]$とし，$X$を$\mathscr{M}_{\mathbf{T}}$の要素とする．このとき，
			$\{\mathscr{F}_t\}_{t \in \mathbf{T}}$-停止時刻の集合$\{\tau_n\}_{n \in \Natural}$で
			\begin{align}
				\forall \omega \in \Omega\, \left(\, \tau_0(\omega) = 0\, \right)
			\end{align}
			かつ
			\begin{align}
				\forall \omega \in \Omega\, \forall n \in \Natural\,
				\left(\, \tau_n(\omega) \leq \tau_{n+1}(\omega)\, \right)
			\end{align}
			かつ
			\begin{align}
				\forall \omega \in \Omega\, \exists n \in \Natural\,
				\left(\, \tau_n(\omega) = T\, \right)
			\end{align}
			かつ$X^{\tau_n}$を全て有界とするものが取れる．
		\end{thm}
	\end{screen}
	...
	\begin{sketch}\mbox{}
		\begin{description}
			...
			\item[step1-4]
				...
				\end{align}
				が成り立つ．なぜならば(\refeq{fom:thm_decomposition_of_local_martingales_3})より
				\begin{align}
					X^2 - h(n_k) \in \conv{\Set{A^{(j)}}{j \in \Natural \wedge n_k \leq j}}
				\end{align}
				となるためである．そして(\refeq{fom:thm_decomposition_of_local_martingales_4})と併せて
				\begin{align}
				...
				\end{align}
				が成り立つので(\refeq{fom:thm_decomposition_of_local_martingales_4})から
				\begin{align}
