	...
	\subsection{正則性}
		\begin{screen}
			\begin{dfn}[正則正値測度]
				...
				また$\mathcal{B}$を$X$上の$\sigma$-加法族で$\borel{X}$を含むものとする．
				$\mu$を$\mathcal{B}$上の正値測度とするとき，
				\begin{description}
					\item[外正則性] $\mathcal{B}$の要素の測度はそれを含む開集合の測度の下限で得られる:
						\begin{align}
							\forall E \in \mathcal{B}\, \left(\, 
							\mu(E) = \inf{}{\Set{\mu(O)}{O \in \mathscr{O}_X \wedge E \subset O}}\, \right).
						\end{align}
						
					\item[内正則性] 開集合及び測度が有限値である$\mathcal{B}$の要素はそれが含む
						$X$のコンパクト部分集合の測度の上限で得られる:
						\begin{align}
							\forall E \in \mathcal{B}\, \left(\, 
							E \in \mathscr{O}_X \vee \mu(E) < \infty 
							\Longrightarrow
							\mu(E) = \sup{}{\Set{\mu(K)}{K \in \mathscr{K}_X \wedge K \subset E}}\, \right).
						\end{align}
				\end{description}
				が満たされているなら$\mu$は{\bf 正則な}\index{せいそくせいちそくど@正則正値測度}{\bf (regular)}正値測度と呼ばれる．
			\end{dfn}
		\end{screen}
