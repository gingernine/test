\subsection{写像}
	\begin{screen}
		\begin{dfn}[グラフ・写像]
			...
			$\Gamma$を$f$の{\bf グラフ}\index{ぐらふ@グラフ}{\bf (graph)}，$f$を($\Gamma$をグラフとする)
			$A$から$B$への{\bf 写像}\index{しゃぞう@写像}{\bf (mapping)}と呼ぶ．
			また$\Gamma$を$\mathscr{G}(f)$と書いたり，$f$を$f:A \longrightarrow B$や
			$f:a \longmapsto b$と書いたりする．
			$A$を$f$の{\bf 定義域}\index{ていぎいき@定義域}{\bf (domain)}，
			$B$を$f$の{\bf 終集合}\index{しゅうしゅうごう@終集合}{\bf (codomain)}，
			\begin{align}
				f(A) = \Set{b \in B}{\mbox{或る$a \in A$で$(a,b) \in \Gamma$}}
			\end{align}
			を$f$の{\bf 値域}\index{ちいき@値域}{\bf (range)}と呼ぶ．
