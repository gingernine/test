\subsection{局所凸}
	...
	\begin{sketch}
		...
		なる写像を$\varphi$とおけば
		\begin{align}
			\Set{\alpha \in \Phi}{|\alpha| \leq \delta} \subset \varphi^{-1} \ast u
		\end{align}
		を満たす正の実数$\delta$が取れる．
		\begin{align}
			r \defeq \frac{1}{\delta}
		\end{align}
		とおけば
		\begin{align}
			s\left(r^{-1},x\right) \in u
	...
	\begin{screen}
		\begin{dfn}[併呑集合]
			...
			$A$を$X$の部分集合とする．$X$の要素$x$が任意に与えられたとき
			\begin{align}
				s\left(r^{-1},x\right) \in A
			\end{align}
			を満たす正の実数$r$が取れるならば，$A$は$\left(\left(X,\sigma_X\right),(\Phi,+,\bullet),s\right)$の
			{\bf 併呑集合}\index{へいどんしゅうごう@併呑集合}{\bf (absorbing set)}と呼ばれる．
		\end{dfn}
	\end{screen}
