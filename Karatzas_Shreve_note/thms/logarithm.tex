\subsection{対数関数}
	...
	\begin{screen}
		\begin{thm}[$\pvlog$の正則性]\label{thm:pv_log_is_holomorphic}
			$\C$から$0$以下の実数を除いた集合を$\C^*$とすると，つまり
			\begin{align}
				\C^* \defeq \Set{z \in \C}{z \notin \R \vee 0 < z}
			\end{align}
			とおくと，$\pvlog$は$\C^*$上で正則である．また$z$を$\C^*$の要素とすると，
			$z$における$\pvlog$の微分係数は
			\begin{align}
				\frac{1}{z}
			\end{align}
		...
		\begin{description}
			...
			\item[第二段]
				...
				\begin{align}
					0 < |w - \zeta| < \delta
				\end{align}
				が満たされるので，(\refeq{fom:thm_pv_log_is_holomorphic_2})より
				\begin{align}
				...
				\end{align}
				が成り立つ．ところで(\refeq{fom:thm_pv_log_is_holomorphic_3})より
				\begin{align}
