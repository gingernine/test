		...
		\begin{description}
			\item[(1)]
				...
				\begin{align}
					...
					\cdots
				\end{align}
				が構成され，$n(k) \coloneqq n(k,k),\ (\forall k \geq 1)$
				とおけば任意の$x_i \in E$に対して$\left\{f_{n(k)}(x_i)\right\}_{k=i}^\infty
				\subset \left\{f_{n(k,i)}(x)\right\}_{k=1}^\infty$より
				$\left\{f_{n(k)}(x_i)\right\}_{k=i}^\infty$は収束列の部分列となるから収束する．
				従って或る$N \geq 1$が存在して
				\begin{align}
