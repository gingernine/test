\section{連続写像の空間上の完備可分距離}
	...
	\begin{screen}
		\begin{thm}[広義一様収束距離の可分性]
			$(X,d_X)$を$\sigma$-コンパクト距離空間，$(Y,d_Y)$を可分距離空間とするとき，$C(X,Y)$は$\rho$により可分距離空間となる．
		\end{thm}
	\end{screen}
	
	\begin{screen}
		\begin{thm}[広義一様収束距離の完備性]
			$(X,d_X)$を$\sigma$-コンパクト距離空間，$(Y,d_Y)$を完備距離空間とするとき，$C(X,Y)$は$\rho$により完備距離空間となる．
		\end{thm}
	\end{screen}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				$(f_n)_{n=1}^\infty$を$C(X,Y)$のCauchy列として，
				$(K_n)_{n=1}^\infty$を(\refeq{eq:completeness_and_separability_of_space_of_continuous_functions_1})
				を満たすコンパクト部分集合の列とする．
				任意の$x \in X$に対し$x$を含む$K_j$を取れば
				\begin{align}
					1 \wedge d_Y(f_n(x),f_m(x)) \leq 2^j \rho(f_n,f_m) \longrightarrow 0
	...
	\begin{screen}
		\begin{thm}[広義一様収束距離の完備可分性]
			$(X,d_X)$を$\sigma$-コンパクト距離空間，$(Y,d_Y)$を完備可分距離空間とするとき，$C(X,Y)$は$\rho$により完備可分距離空間となる．
		\end{thm}
	\end{screen}