...
\subsection{正規族}
	...
	\begin{screen}
		\begin{thm}[Ascoli-Arzela]
			$(X,d_X)$を可分距離空間，$(Y,d_Y)$を距離空間とするとき，
			$\mathscr{F} \subset C(X,Y)$に対して次が成り立つ:
			\begin{align}
				\mbox{$\mathscr{F}$が正規族} \quad \Longleftrightarrow \quad
				\begin{cases}
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第二段] 
				...
				(\refeq{eq:distance_in_spaces_of_continuous_functions})の距離$\rho$に関して全有界となるから，
				任意の$\epsilon > 0$に対して
				\begin{align}
					\mathscr{F}
