...
\subsection{言語}
	...
	\monologue{
		...
		``$A$が成り立つ''や``$A$は正しい''や``$A$は真である''などとも書かれます．」
	}
	...
	\begin{screen}
		\begin{metaaxm}[量化記号]
			...
			\begin{description}
				...
				\item[存在記号の規則] $\tau$を$\mathcal{L}$の対象とするとき
	...
	\begin{prf}
		$\tau$を$\mathcal{L}$の任意の対象とすれば，存在記号に関する規則より
		\begin{align}
			\begin{gathered}
			...
			\end{gathered}
		\end{align}
		が成り立つから，推論規則\ref{metathm:transitive_law_of_implication}より
		...
		\begin{align}
			...
			\Longrightarrow A(\tau)
		\end{align}
		が成り立つ．全称記号に関する規則より$A \left( \varepsilon x \rightharpoondown A(x) \right)$と
