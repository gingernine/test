...
\section{言語}
	...
	自然言語において，``あｍｔ後右所ｓごぐふぉｓｄ''のように無作為に文字を並べただけでは意味不明な
	...
	\begin{itembox}[l]{`以上の操作を繰り返して得られる記号列のみが式である'の意味}
		...
		最後の規制によりこれは式ではないと決めているのです．%具体的な問題を考える際は上の例のような式は扱わないので，
...
\section{推論}
	...
	\monologue{
		院生「定義記号$\defeq$と同様に，`$A \defarrow B$'とは
	...
	\monologue{
		院生「以上で``集合であり真類でもある類は存在しない''という言明を証明する準備が整いました．」
	}
	...
	\monologue{
		院生「次は量化記号が推論操作の上でどのような働きを持つのかを規定しましょう．」
	}
	...
	\monologue{
		院生「推論法則\ref{logicalthm:fundamental_law_of_universal_quantifier}を根拠にして，
