...
\subsection{言語}
	...
	\monologue{
		院生「定数$\emptyset$は$\mathcal{L}$の対象の一つです．
			...
			また文字は対象ではないということも認めます．説明中は``$s$を対象とする''のように書かれることが多いですが，
			これは一時的に$s$を対象の一つに代用しているだけで，文字$s$が対象であると言っているのではありません．
			そもそも先に約束した通り文字は$\mathcal{L}$の対象ではありませんね．
	...
	\begin{description}
		...
		\item[式] 
			\begin{itemize}
				...
				\item $A$を$\mathcal{L}'$の式とするとき，文字$x$が$A$に現れ，かつ$x$が$A$で量化されていないときに限り
	...
	\monologue{
		院生「$A$を$\mathcal{L}$の式とするとき，$x$が$A$に現れ，かつ$x$のみが$A$で量化されていないならば
			...
			これらを同等な対象として扱いたいものです．実際このことは後述する``類の公理''と``外延性の公理''により保証されます．」
	}
	...
	一度枠線付きの定義で名前を付けられた対象や概念は，それ以後は本稿においてはその名前で通用する．
	...
	\begin{screen}
		\begin{thm}[集合とは，$\mathcal{L}$の対象か，或いは$\mathcal{L}$の対象と等しい類のことを指す]
