	...
	\begin{itembox}[l]{研究室にて}
		院生「私の指導教官に``新約聖書がはじめにギリシア語で書かれたとき，`ことば'にはlogosが充てられた．
			...
			一方で日本語訳では言葉ではなく言と書かれていますね．なぜ``言葉''ではなく``言''と書くのでしょうか．
	...
	\begin{screen}
		\begin{dfn}[変項]
			言語で扱われる英字とギリシア文字\footnotemark
			のうち，対象領域にもいずれの対象にも
			あてがわれておらず，かつデフォルトのフォントであるものを{\bf 変項}\index{へんこう@変項}{\bf (variable)}と呼ぶ．
		\end{dfn}
	\end{screen}
	
	\footnotetext{数学で扱われる文字は英字かギリシア文字である．}
	...
	\begin{itembox}[l]{研究室にて}
		院生「今後は$\emptyset$と${\boldsymbol\omega}$が特別な記号として出てくるでしょう．
			...
			(しかしながら確率論では$\omega$がやたらと使われます．先に書いた${\boldsymbol\omega}$は最小の極限数を表す記号で，
			違いを出すために太字にしているのですが見分けづらい...でも確率論での$\omega$と極限数としての${\boldsymbol\omega}$が
			同時に登場する場面はあまりないと思われますからそれほど気にしないで良さそうですね．)
			また$\N$や$\R$など太字で特別な意味を持つようになる文字もありますから，
			変項は`デフォルトのフォントであるもの'と現代的な制限を課しています．大抵の文字は特別な意味を持っていないので変項として扱って問題ありません．
	...
	\begin{screen}
		\begin{dfn}[命題]
			自由変項を含まない式を{\bf 閉論理式}\index{へいろんりしき@閉論理式}{\bf (closed formula)}，
			或は{\bf 命題}\index{めいだい@命題}{\bf (proposition)}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	\begin{description}
		\item[対象]
			\begin{itemize}
				...
				\item 項$x$のみを自由変項とする$\mathcal{L}$の論理式$A(x)$に対し
	...
	\begin{itembox}[l]{研究室にて}
		院生「宇宙という壮大な言葉が出てきてしまいました．現実世界において人間が把握し得る最大の世界は宇宙空間でしょう．
			定理\ref{}まで読めばわかるように集合論の世界は$V$の範囲内で語り尽くせてしまうのですから，現代数学にとって$V$は宇宙そのものなのですね．
			と考えますと，数学の世界では宇宙の外側を見ることが出来るのです．それは真類と呼ばれます．
			真類としてはRusselのパラドックスが有名ですが，後述にまかせましょう．」
	\end{itembox}
	...
	\begin{itembox}[l]{研究室にて}
		院生「類は$V$の要素であれば集合と呼ばれ，$V$の要素でなければ真類と呼ばれます．
			...
			我々は，数学において中庸が無いということや矛盾が起きるということをどう表現するべきかという問題に直面しているのです．
			ここで{\bf 推論規則}\index{すいろんきそく@推論規則}{\bf (rule of inference)}と言われるものを導入します．」
	\end{itembox}
	...
	\begin{itembox}[l]{研究室にて}
		院生「類$a$が集合であるということは$a \in V$という式で表現できるのだ．そして真類とは，集合でない類のことを指すのだね．」
	\end{itembox}
	...
	\begin{itembox}[l]{研究室にて}
		院生「現代的な数学では，数や関数など数学に関するあらゆるものは集合で構成されます．
			...
			関数や$k$項述語などといったものを取り扱っているものばかりであるのが残念です．
			モノの数え方すら知らない生まれたばかりの世界に立って色々説明しようというのに，
			その前準備の段階で数の存在を予定したり関数の概念を前借りしてしまうのは好ましくないし，
			そのように集合論と論理学が循環しているのは勉強する側にとってはただの障壁にしかなりません．
			どのみち両者は切り離せないのですから，同時に並行して記述する方がやりやすいのではないでしょうか．」
	\end{itembox}
	...
	\begin{screen}
		\begin{thm}[空集合はただ一つ]
			空集合の存在公理を満たす集合はただ一つである．つまり次が成立する:
			\begin{align}
				\forall x \in V \forall y \in V
				((\forall z \in V(z \notin x) \wedge \forall z \in V(z \notin y))
				\Longrightarrow x=y).
			\end{align}
	...
	\begin{itembox}[l]{研究室にて}
		院生「はじめのうちは空論が続いていたようですけれど，ようやく存在が約束された本物の集合が出てきましたね．
