...
\subsection{言語}
	...
	\monologue{
		院生「我々は式を作る際に$\forall$と$\exists$を量化記号として採用していましたから，
			次は量化記号が推論操作の上でどのような意味を持つのかを規定しましょう．」
	}
	...
	\begin{screen}
		\begin{metaaxm}[量化記号]
			...
			\begin{description}
				\item[$\varepsilon$記号の導入] $\varepsilon x A(x)$は$\mathcal{L}$の対象である．
				...
				\item[存在記号の公理] $\tau$を$\mathcal{L}$の対象とするとき
					\begin{align}
