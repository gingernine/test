...
\subsection{言語}
	...
	\monologue{
		院生「対象がどういうものであるかは後で判明しますが，
			今のところはその正体は伏せておいて，とりあえず対象は予め存在しているものとして話を進めます．
			また{\bf $\mathcal{L}$の対象は記号列で表せない}ということも認めます．
			$\emptyset$を定数としましたが，これは記号`$\emptyset$'が$\mathcal{L}$の或る特別な対象に付けられた
			名称であるという意味です．見上げる太陽そのものと呼び名である文字列`太陽'との関係のようです．
			また説明中は``$s$を対象とする''のように書くことが多いですが，
	...
	\monologue{
		院生「$A$を$\mathcal{L}$の式とするとき，文字$x$が$A$に現れ，かつ$x$のみが$A$で量化されていないならば
			\begin{align}
				\Set{x}{A(x)}
			\end{align}
	...
	\monologue{
		院生「$\Univ$はそもそも式に現れる記号ではありませんでしたが，$\defeq$の推薦で$\Set{x}{x=x}$の代理人として市民権を得たのですね．
			...
			ちなみに，宇宙が$\Univ$で表されるのはJohn Von NeumannのVに由来しています．」
	}
	...
	\monologue{
		院生「我々はまだ$\exists$が持つ意味を規定していませんが，先に述べてしまうと上の定義は
	...
	\begin{screen}
		\begin{metaaxm}[矛盾と否定に関する規則]\label{metaaxm:rules_of_contradiction}
	...
	否定の導出の逆は定理として得られる．
	\begin{screen}
		\begin{metathm}[否定が正しい式は偽である]\label{metathm:false_and_negation_are_equivalent}
	...
	\begin{screen}
		\begin{metathm}[全称記号と任意性]\label{metathm:fundamental_law_of_universal_quantifier}
