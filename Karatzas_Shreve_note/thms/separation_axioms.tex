\subsection{分離公理}
	...
	\begin{prf}
		...
		\end{align}
		となる．ただし上線は$S$における閉包を表す．
		$S$は$T_1$であるから，任意に$a \in A$と$b \in B$を取れば$\{b\}$と$\overline{A}$，
		$\{a\}$と$\overline{B}$はそれぞれ互いに素な$S$の閉集合の組となり，完全正規性より
		\begin{align}
			\overline{A} = f^{-1}(\{0\}),
			\quad \overline{B} = g^{-1}(\{0\})
		\end{align}
		を満たす連続写像$f,g:S \longrightarrow [0,1]$が存在する．
		...
		\begin{align}
			...
			\end{cases}
		\end{align}
		が成り立つ．$A \subset T \cap h^{-1}((-\infty,0))$かつ
