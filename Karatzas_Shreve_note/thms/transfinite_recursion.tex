	...
	\begin{screen}
		\begin{dfn}[構成可能集合]
			$\bigcup_{\alpha \in \ON} R(\alpha)$の要素を{\bf 構成可能集合}
			\index{こうせいかのうしゅうごう@構成可能集合}{\bf (constructible set)}と呼ぶ．
		\end{dfn}
	\end{screen}
	
	\begin{screen}
		\begin{thm}[宇宙は構成可能集合で尽くされる]\label{thm:Universe_and_ordinal_numbers}
			次は定理である:
			\begin{align}
	...
	\monologue{
		院生「\begin{align}
			...
			予定して正則性公理を導入したのです．
			その事実および証明は竹内集合論\cite{key4}の第2章にあります．
