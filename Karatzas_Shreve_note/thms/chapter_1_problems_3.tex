...
\subsection{Fundamental Inequalities}
	...
	\begin{prf}
		...
		十分大きい$n \in \N$に対して，或る$t_i,s_i \in F_n,\ (1 \leq i \leq j)$が
		\begin{align}
		...
		\end{align}
		が従い，$\omega$の任意性より$U_F(\alpha,\beta;X(\omega)) \leq \sup{n \in \N}{U_{F_n}(\alpha,\beta;X)}$が出る．
		\QED
	\end{prf}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				任意の有限部分集合$F \subset [\sigma,\tau]$に対し
				\begin{align}
					E U_F(\alpha,\beta;X) \leq \frac{E(X_\tau^+) + |\alpha|}{\beta-\alpha}
				\end{align}
				が成立することを示せば，補題の$(F_n)_{n=1}^\infty$に対し
				\begin{align}
					E U_{F_n}(\alpha,\beta;X)
					\leq E U_{F_{n+1}}(\alpha,\beta;X),
					\quad (n=1,2,\cdots)
				\end{align}
				より単調収束定理から
				\begin{align}
