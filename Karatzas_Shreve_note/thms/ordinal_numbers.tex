\subsection{順序数}
	...
	\begin{itemize}
		...
		\item $a \subset \ON$なら，$\bigcup a$は$a$の順序$R$に関する上限となる．
		\item $\ON$は集合ではない．
	\end{itemize}
	
	
	\begin{screen}
		\begin{thm}[順序数の諸性質]\label{thm:properties_of_ordinal_numbers}\mbox{}
			\begin{description}
				\item[(i)] $\forall \alpha\ (\ \alpha \in \ON \Longrightarrow \alpha \subset \ON\ )$
		
				\item[(ii)] $a \subset \ON \wedge a \neq \emptyset 
					\Longrightarrow \exists x \in a\ \forall y \in a\ (\ x \subset y\ )$.
		
				\item[(iii)] $\forall a\ (\ a \subset \ON \Longrightarrow \bigcup a \in \ON\ )$
		
				\item[(iv)] $\forall a,b \in \ON\ (\ a \in b \Longleftrightarrow a \subsetneq b\ )$
				
				\item[(v)] $\forall a \in \ON\ (\ a \neq \emptyset \Longrightarrow \emptyset \in a\ )$
		
				\item[(vi)] $R = \Set{x}{\exists a,b \in \ON\ (\ x=(a,b) \wedge a \subset b\ )}$は全順序
	...
	\begin{screen}
		\begin{thm}[$\ON$の整列性]
			$\ON$の任意の部分集合は次の意味で最小元を持つ:
			\begin{align}
				\forall a\ \left(\ a \subset \ON \wedge a \neq \emptyset 
				\Longrightarrow \exists x \in a\ \forall y \in a\ (\ x \subset y\ )\ \right).
			\end{align}
		\end{thm}
	\end{screen}
