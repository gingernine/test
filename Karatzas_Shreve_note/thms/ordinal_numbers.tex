\subsection{順序数}
	...
	\begin{screen}
		...
		\label{thm:no_set_is_an_element_of_itself}
			$a,b,c$を類とするとき次が成り立つ:
			\begin{itemize}
				\item $a \in \Univ \Longrightarrow a \notin a$.
				
				\item $a,b \in \Univ \Longrightarrow a \notin b \vee b \notin a$.
				
				\item $a,b,c \in \Univ \Longrightarrow a \notin b \vee b \notin c \vee c \notin a$.
			\end{itemize}
		\end{thm}
	\end{screen}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段] $a$を集合とする．$a = \emptyset$ならば，空集合の公理より
				\begin{align}
					\emptyset \notin \emptyset
				\end{align}
				が成立するので$a \notin a$が従う．
				$a \neq \emptyset$ならば，正則性公理より$\mathcal{L}$の或る対象$\tau$が存在して
				\begin{align}
					\tau \in \{a\} \wedge \tau \cap \{a\} = \emptyset
				\end{align}
				を満たす．$\tau \in \{a\}$より$\tau = a$となるので
				\begin{align}
					a \cap \{a\} = \emptyset
				\end{align}
				が従い，$a \in \{a\}$も満たされるから
			...
			\item[第二段]
		\end{description}
		また$x \in y$が満たされていると仮定すれば，$y \cap \{x,y\} \neq \emptyset$より
		\begin{align}
			x \cap \{x,y\} = \emptyset
		\end{align}
		が成り立つから$y \notin x$が従い
