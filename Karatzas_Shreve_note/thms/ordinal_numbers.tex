\subsection{順序数}
	...
	\begin{prf}
		いま$\set{\ON}$が成立していると仮定する．このとき$\exists x\, (\, \ON = x\, )$が成立するので
		\begin{align}
			\tau \coloneqq \varepsilon x\, (\, \ON = x\, )
		\end{align}
		とおけば$\ON = \tau$となる．よって相等性の公理より
		\begin{align}
			\ord{\tau}
		\end{align}
		が成立し，他方で類の公理と推論法則\ref{metathm:fundamental_law_of_universal_quantifier}より
		\begin{align}
			\ord{\tau} \Longleftrightarrow \tau \in \ON
		\end{align}
		が満たされているので$\tau \in \ON$が従う．再び相等性の公理を適用して
		\begin{align}
			\ON \in \ON
		\end{align}
		が成立し，ここで演繹法則を適用して
		\begin{align}
			...
			\label{eq:Burali_Forti}
		\end{align}
		が得られる．ところで定理\ref{thm:no_set_is_an_element_of_itself}より$\ON \notin \ON$が成り立っているので，
		式(\refeq{eq:Burali_Forti})の対偶と併せて
		\begin{align}
