\subsection{The Optional Sampling Theorem}
	\begin{itembox}[l]{Lemma: 離散時間の任意抽出定理}\label{lem:optional_sampling_theorem}
		...
		$S,T:\Omega \longrightarrow \{t_0,t_1,\cdots,t_n,\infty\}$を$(\mathscr{F}_{t_i})$-停止時刻，
	...
	\end{itembox}
	\footnotetext{
		$\tilde{X}_0 = 0\ \mbox{a.s. $P$}$だと$X_t = \tilde{X}_{(t-T) \vee 0}$が
		$(\mathscr{F}_t)$-適合となるかわからない．
	}
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段] $\inf{}{\Set{t \in [s,\infty)}{Z_t(\omega) = b}} = \inf{}{\Set{t \in [0,\infty)}{Z_{t+s}(\omega) = b}} + s$と
				...
				$T(\omega) \leq t$が従い(\refeq{eq:chapter_1_Problem_3_28_2})が得られる．
