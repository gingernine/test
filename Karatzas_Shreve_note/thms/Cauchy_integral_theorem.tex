\subsection{積分定理}
	...
	\end{align}
	と定める．$\seg{z}{w}$とは$z$から$w$に直進する線分を描き，また$f$をその線分上の連続写像とすれば
	\begin{align}
	...
	\begin{screen}
		\begin{thm}[Goursatの定理]\label{Cauchy_Goursat_theorem}
			$a$と$b$と$c$を複素数とし，
			\begin{align}
				\Delta \defeq \Set{z}{\exists t,s \in [0,1]\, 
				\left(\, z = (1-t) \cdot a 
			...
			かつ
			\begin{align}
				f \in \Holomorphic{\Omega \backslash \{p\}}
			\end{align}
			であれば
			\begin{align}
	...
	\begin{screen}
		\begin{thm}[円周上の積分公式]
			...
			なる写像とすれば，
			\begin{align}
				f(z) = \frac{1}{2 \cdot \pi \cdot \isym} \cdot \int_{\gamma} \frac{f(\xi)}{\xi - z}\ d\xi
			\end{align}
			が成立する．
		\end{thm}
	\end{screen}
	...
	\begin{sketch}
		...
		\begin{align}
			\Omega \ni \xi \longmapsto
			\begin{cases}
				\displaystyle{\frac{f(\xi) - f(z)}{\xi - z}} & \mbox{if } \xi \neq z \\
				f'(z) & \mbox{if } \xi = z
			\end{cases}
		\end{align}
