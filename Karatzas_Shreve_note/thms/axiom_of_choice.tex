	\begin{screen}
		\begin{dfn}[直積]
			...
			\end{align}
			と定め，これを$h$の{\bf 直積}\index{ちょくせき@直積}{\bf (direct product)}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	\monologue{
		院生「選択公理をもって本稿の公理は出尽くしました．次の整列可能定理は
			いかなる集合の上にも整列順序が定まることを保証するものです．主張は
			\begin{align}
			...
			\end{align}
			で$a$上の関係を定めれば，$R$は$a$上の整列順序となります．」
	}
		...
		\begin{description}
			...
			\item[第三段]
				...
				\begin{align}
					\set{F \ast S}
				\end{align}
				が出る．次に$F|_S$が単射であることを示す．$\beta,\gamma \in S$に対し，
				$\beta \neq \gamma$ならば
				\begin{align}
			...
			\item[第四段]
				...
				\end{align}
				とおけば式(\refeq{eq:thm_well_ordering_theorem_1})より
				\begin{align}
