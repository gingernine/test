\subsection{級数}
	...
	\end{align}
	なる級数が$\C$で存在しているなら，これを$a$と$b$の{\bf Cauchy積}\index{Cauchy積}{\bf (Cauchy product)}と呼ぶ．
	
	\begin{screen}
		\begin{thm}[一方が絶対収束していればCauchy積も収束する(Mertens)]\label{thm:convolution_of_absolutely_convergent_series}
			...
			\end{align}
			なる写像とする．このとき$\sum_{n=0}^\infty a_n$が$\C$で絶対収束して，
