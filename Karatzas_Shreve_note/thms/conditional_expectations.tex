\section{条件付き期待値}
	...
	\begin{screen}
		\begin{thm}[$L^p$の共役空間]\label{thm:dual_space_of_L_p}
			...
			\end{align}
			の意味で等長であり，$\Phi$により$\left( L^p(\mu) \right)^*$と$L^q(\mu)$はBanach空間として同型となる．
		\end{thm}
	\end{screen}
	...
	\begin{screen}
		\begin{thm}[条件付き期待値の性質]\label{thm:properties_of_conditional_expectations}
			...
			\begin{description}
				...
				\item[(4)] $f \in L^1(\mu) \cap L^\infty(\mu)$なら
					\begin{align}
						\Norm{\cexp{f}{\mathscr{G}}}{L^\infty(\mu)}
						\leq \Norm{f}{L^\infty(\mu)}.
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[(4)] 任意の$A \in \mathscr{G}$に対して(\refeq{eq:thm_properties_of_conditional_expectations_2})より
				\begin{align}
				...
				\end{align}
				が成り立つから(\refeq{eq:thm_properties_of_conditional_expectations_3})が従う．
				\QED
		\end{description}
	\end{prf}
