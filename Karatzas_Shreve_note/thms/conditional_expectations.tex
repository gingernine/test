\section{条件付き期待値}
	...
	\begin{prf}
		(\refeq{eq:thm_Vitali_Hahn_Saks})が複素数値で確定しているとき，$(X,\mathscr{F})$上の有限測度を
		\begin{align}
			\mu \coloneqq \sum_{n=1}^\infty \frac{1}{2^n(1 + \Norm{\lambda_n}{})} |\lambda_n|
		\end{align}
		により定めれば$\lambda_n \ll \mu\ (\forall n \geq 1)$より
		$\lambda \ll \mu$となる．任意の互いに素な$A,B \in \mathscr{F}$を取れば
		\begin{align}
			\lambda(A + B) = \lim_{n \to \infty} \lambda_n(A + B)
			= \lim_{n \to \infty} \lambda_n(A) + \lim_{n \to \infty} \lambda_n(B)
			= \lambda(A) + \lambda(B)
		\end{align}
		が成り立つから$\lambda$は有限加法的であり，このとき任意の互いに素な列$\{E_i\}_{i=1}^\infty \subset \mathscr{F}$に対し
		\begin{align}
		...
		\end{align}
		と定理\ref{thm:equivalent_condition_of_absolute_continuity}より
		\begin{align}
