\section{証明}
	...
	本稿では証明された文を{\bf 真な}\index{しん@真}{\bf (true)}文と呼ぶことにするが，
	``証明された''や``真である''という状態は議論が立脚している前提に依存する．
	ここでいう前提とは，推論規則や言語ではなくて
	{\bf 公理系}\index{こうりけい@公理系}{\bf (axioms)}と呼ばれるものを指している．
	
	$\mathscr{S}$を公理系とするとき，$\mathscr{S}$に属する文を$\mathscr{S}$の{\bf 公理}\index{こうり@公理}{\bf (axiom)}
	...
	$\mathscr{S}$を公理系とするときに，文$\varphi$が$\mathscr{S}$から{\bf 証明可能である}
	\index{しょうめいかのう@証明可能}{\bf (provable)}とは，
	\begin{itemize}
		\item $\varphi$は$\mathscr{S}$の公理である．
		\item $\vdash \varphi$である．
		\item 閉式$\psi$で，$\psi$と$\psi \rightarrow \varphi$が$\mathscr{S}$から証明可能である
			ものが取れる．
	\end{itemize}
