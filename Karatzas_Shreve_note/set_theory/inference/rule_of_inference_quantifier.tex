	\begin{screen}
		...
		\label{logicalaxm:rules_of_quantifiers}
			...
			$A$には$x$のみが自由に現れるとする．
			また$\tau$を主要$\varepsilon$項とする．このとき以下を推論公理とする．
			\begin{align}
			...
			$A$が$\lang{\varepsilon}$の式であるときは$A$そのものである．
		\end{logicalaxm}
	\end{screen}
	...
	\end{align}
	が従う．そして対偶律4 (推論法則\ref{logicalthm:proof_by_contraposition})より
	\begin{align}
	...
	\begin{comment}
		...
		\begin{align}
			\vdash\ \negation \exists x \negation A(x) \rarrow\ \negation\negation A(\varepsilon x \negation \hat{A}(x))
		\end{align}
		が従い(推論法則\ref{logicalthm:introduction_of_contraposition})，演繹定理の逆より
		\begin{align}
			\negation \exists x \negation A(x) \vdash\ \negation\negation A(\varepsilon x \negation \hat{A}(x))
		\end{align}
		となる．そして二重否定の除去より
