	...
	集合は特定の性質を持つクラスとして定義されるが，クラスが全て集合であると考えると矛盾が起こる．
	たとえばRussellのパラドックスで有名な
	\begin{align}
		R \defeq \Set{x}{x \notin x}
	\end{align}
	なるクラスが集合であるとすると($\defarrow$は``式''に対する略記の導入に使ったが
	(P. \pageref{fom:defining_arrow})，
	これと同様に$\defeq$とはクラスに対する略記を導入するために使う定義記号である)
	\begin{align}
	...
	大切なのは「要素になれるクラス」を制限することなのである(Morse\cite{Morse} P. xx)．
	
	\begin{screen}
		\begin{dfn}[集合]
			$a$をクラスとするとき，「$a$が集合である」という式を
			\begin{align}
			...
			{\bf 真クラス}\index{しんくらす@真クラス}{\bf (proper class)}と呼ぶ．
		\end{dfn}
	\end{screen}
...
\section{相等性}
	...
	本節では，いつクラスは等しくなるのか，そして，等しい場合に何が起きるのか，の二つが主題となる．
	
	\begin{screen}
		\begin{axm}[外延性の公理 (Extensionality)]
			$a$と$b$をクラスとするとき次の式を$\EXTAX$により参照する：
			\begin{align}
	...
	\begin{screen}
		\begin{thm}[任意のクラスは自分自身と等しい]\label{thm:any_class_equals_to_itself}
			$a$をクラスとするとき
			\begin{align}
				\EXTAX \vdash a = a.
			\end{align}
	...
	\begin{screen}
		\begin{axm}[相等性公理]
			$a,b,c$をクラスとするとき次の式を$\EQAX$により参照する：
			\begin{align}
				a = b &\rarrow b = a,  \\
	...
	\begin{prf}
		...
		\begin{align}
			a=b,\ \EQAX \vdash \forall x\, (\, x \in a  \lrarrow x \in b\, )
		\end{align}
		となるが，ここでクラスである任意の$\varepsilon$項$\tau$に対して
		\begin{align}
			a=b,\ \EQAX \vdash \tau \in a \lrarrow \tau \in b
		\end{align}
	...
	\begin{screen}
		\begin{thm}[条件を満たす集合は要素である]\label{thm:satisfactory_set_is_an_element}
			...
			$\varphi$には$x$のみが自由に現れているとする．このとき，任意のクラス$a$に対して
			\begin{align}
	...
	\begin{screen}
		\begin{dfn}[宇宙]
			$\Univ \defeq \Set{x}{x=x}$で定めるクラス$\Univ$を{\bf 宇宙}\index{うちゅう@宇宙}
			{\bf (Universe)}と呼ぶ．
		\end{dfn}
	\end{screen}
	
	宇宙とは集合の全体を表すが，これ自体は集合ではない．
	ここで$\Univ$が集合の全体を表すとは，任意のクラス$a$に対して
	...
	要素の公理は{\bf 要素となりうるクラスは集合である}と規制している．
