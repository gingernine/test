...
\label{sec:equivalence_of_formula_rewriting}
	...
	\end{align}
	とおけば(\refeq{formula_rewriting_1})より
	\begin{align}
	...
	一般の文$\varphi$に対して(\refeq{formula_rewriting_1})を示すには
	$\varphi$が原子式であるとして(\refeq{formula_rewriting_1})を示せば殆ど十分であり，
	それについて先に結論を書いておくと
	\begin{align}
	...
	\begin{itembox}[l]{$\mathcal{L}$の式を用いた$\varepsilon$項，内包項}
		...
		書き換えは同値なので
		\begin{align}
			\forall x\, (\, x \in \Set{x}{\varphi(x)} \lrarrow 
			\widehat{\varphi}(x) \lrarrow \varphi(x)\, )
		\end{align}
		が満たされる．
	\end{itembox}