...
\subsection{代入}
	変項とは束縛されうる項であったが，代入可能な項でもある．
	代入とはすなわち別の項で置き換えるということである．
	たとえば
	\begin{align}
		x \in \forall y\, (\, y = y\, )
	\end{align}
	といった式(中置記法)では$x$は自由に現れているが，この$x$に$z$を代入すれば
	\begin{align}
		z \in \forall y\, (\, y = y\, )
	\end{align}
	となる．では
	\begin{align}
		x \in \forall y\, (\, y = z\, )
	\end{align}
	という式ではどうなるかというと，今度の式には$x$と$z$が自由に現れていることになり，
	$x$に$z$を代入すれば
	\begin{align}
		z \in \forall y\, (\, y = z\, )
	\end{align}
...
\subsection{類}
	...
	意味が定まらなくなる．というのも，変項とは代入可能な項であるから，$y$に代入する閉項ごとに
	$\varepsilon x \varphi$と$\Set{x}{\varphi}$は別の意味を持ち得るのである．
	また閉項であっても意味不明な項もある．たとえば，$\psi$が文であるときに
	\begin{align}
		\varepsilon y \psi
	\end{align}
	や
	...
	導入時にこれらを排除する面倒を避けたからであり，また排除する必要もない．
	
	ただし導入の意図に適っている閉項は特別の名前を持って然るべきである．
...
\subsection{拡張$\varepsilon$項}
	...
	だが$\mathcal{L}$の式の存在文も往々にして登場するので
	そういった場合でも証人を用意できると便利である．
	そこで$\mathcal{L}$の式を``同値な''$\lang{\varepsilon}$の式に書き換えて，
	...
	\end{align}
	を保証するのである．``同値''というのは後述の\ref{sec:equivalence_of_formula_rewriting}	節
	で述べてあるような意味であるが，直感的な意味に即して考えれば次の式の書き換え
