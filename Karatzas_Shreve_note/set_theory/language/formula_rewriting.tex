...
\label{subsec:formula_rewriting}
	...
	\begin{itemize}
		...
		\item (4)の「変項の名前替え」について．
			$\psi$の中の$z$の自由な出現が$\psi$の$\forall a \chi$なる形の部分式の中にある場合に，
			その部分式を$\forall b \chi(a/b)$なる式で置き換える．
			この$b$としては，$\chi$に自由に現れず，かつ$\chi$の中で$a$への代入について自由である変項を取る．
			たとえば
			\begin{align}
			...
			\begin{align}
				\exists b\, (\, a \in b\, )
			\end{align}
			とすればよい．$b$の選び方の条件は，$\forall a \chi$或いは$\exists a \chi$に自由に現れる項を束縛しないためにある．
	\end{itemize}
	
	\begin{screen}
		\begin{metadfn}[式の書き換え]
			$\varphi$を$\lang{\varepsilon}$の式ではない$\mathcal{L}$の式とするとき，$\varphi$の{\bf 書き換え}とは，
			$\varphi$の部分式のうち原子式であるものを全て上表に従って直した式のこととする．
		\end{metadfn}
	\end{screen}
