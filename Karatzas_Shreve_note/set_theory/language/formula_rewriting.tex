...
\label{subsec:formula_rewriting}
	...
	\begin{itemize}
		...
		\item (4)の「変項の名前替え」について．$\psi$の中の$z$の自由な出現が，$\psi$の
			...
			%その直部分式(定義\ref{metadfn:L_immediate_subformula})を差し替える．
			%うちまずは最も狭いものを取り，
	...
	まずは，便宜上の措置として$\xi_{i}$を$\xi_{i}^{1}$と書き($1 \leq i \leq n$
	\footnote{
	...
	%或いはそのような$\psi_{2}$の部分式を部分式として持つような式(例えば$\xi_{3}$が$\xi_{1}$の部分式である場合)は
	\begin{align}
	...
	\footnotetext{
		仮に$\forall x \xi$ (resp. $\exists x \xi$)が$\varphi$の中で複数個所に現れている
		としても，差し替えるのはどれか一か所のみとする．
	}
	
	便宜上，$\forall x \xi$や$\exists x \xi$の形の部分式を
	{\bf 量化部分式}\index{りょうかぶぶんしき@量化部分式}と呼ぶことにする．
	...
	\begin{screen}
		...
		\label{metathm:formula_rewritings_coincide_except_the_differences_of_bound_variables}
			...
			このとき，$\widehat{\varphi}$の量化部分式を複数回だけ差し替えれば$\widetilde{\varphi}$が得られる．
		\end{metathm}
	\end{screen}
	...
	\begin{sketch}\mbox{}
		\begin{description}
			...
			\item[step2]	
				$\varphi$の部分式のうち原子式であるものが$\psi_{1},\cdots,\psi_{k}$で全てであるとし，
				$\widehat{\varphi}_{1}$は各$\psi_{i}$を表\ref{tab:formula_rewriting}に従って
				$\widehat{\xi}_{i}$に書き換えた式であるとし，$\widetilde{\varphi}_{1}$は各$\psi_{i}$を
				表\ref{tab:formula_rewriting}に従って$\widetilde{\xi}_{i}$に書き換えた式であるとする．
				$\psi_{i}$が表\ref{tab:formula_rewriting}の(4)以外の式であれば，$\widehat{\psi}_{i}$と
				$\widetilde{\psi}_{i}$の違いは書き換え時の変項の取り方の違いでしかないから，
				$\widehat{\varphi}_{1}$の$\widehat{\psi}_{i}$の部分を$\widetilde{\psi}_{i}$に
				差し替えることは量化部分式の差し替えである．$\psi_{i}$が
				\begin{align}
					a \in \Set{z}{\psi}
				\end{align}
				なる式で，$a$が変項である場合は，場合によっては$\psi$を書き換えねばならないので厄介である．
