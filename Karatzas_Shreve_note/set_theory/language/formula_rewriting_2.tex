			...
			\item[step1] $\varphi$が原子式であるとする．
				\begin{description}
							...
							\item $x$と$z$が違うとする．このとき
								\begin{itemize}
									\item $x$が$\Set{z}{\psi}$に自由に現れている場合．
										...
										\end{align}
										としよう(注意\ref{rem:rewriting_of_the_4th_kind}参照)．
										ここで$\psi_{n}$とは$\widetilde{\psi}$のことである．
					...
					\item[case7] $\varphi$が
						...
						或る$n$回だけ差し替えた式であり，注意\ref{rem:rewriting_of_the_4th_kind}
						の記号を借りれば，$\psi$から$\widetilde{\psi}$に至るまでの履歴
						\begin{align}
							\psi_{1},\ \psi_{2},\ \cdots,\ \psi_{n}
						\end{align}
						が取れる．ただし$\psi_{n}$は$\widetilde{\psi}$であり，$\psi_{1}$は
						$\psi$の$\xi_{1}^{1}$を差し替えた式
						(記法は注意\ref{rem:rewriting_of_the_4th_kind}参照)，
						$\psi_{i}$は$\psi_{i-1}$の$\xi_{i}^{i}$を差し替えた式である．
						すると，メタ定理\ref{metathm:subformula_replacing_and_substitution}
						より，$\psi_{1}(x/\tau)$は$\psi(x/\tau)$の$\xi_{1}^{1}(x/\tau)$或いは
						$\xi_{1}^{1}$を差し替えた式であり($\psi$の中での$x$の自由な出現が
						$\xi_{1}^{1}$の内部にあれば$\xi_{1}^{1}(x/\tau)$を差し替えることになり，
						そうでなければ$\xi_{1}^{1}$を差し替えることになるが，この辺の説明は
						メタ定理\ref{metathm:subformula_replacing_and_substitution}の脚注にある)，
						$\psi_{i}(x/\tau)$は$\psi_{i-1}(x/\tau)$の$\xi_{i}^{i}(x/\tau)$
						或いは$\xi_{i}^{i}$を差し替えた式である．つまり，$\widetilde{\psi}(x/\tau)$は
						$\psi(x/\tau)$の量化部分式を差し替えていった式である．また差し替えられるのは，
						$\psi(x/\tau)$の中の$z$の自由な出現を内部に持つ$\psi(x/\tau)$の部分式のうち，
						$\forall a$或いは$\exists a$から始まるものの全て，及びそれらのみである．
						%$\widetilde{\psi}(x/\tau)$もまた
