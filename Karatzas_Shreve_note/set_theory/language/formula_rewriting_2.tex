		...
		\label{metathm:subformula_replacing_and_substitution}
			...
			\begin{description}
				\item[(1)] $\varphi$における$x$の自由な出現が差し替えられる$\forall z \xi$ 
					(resp. $\exists z \xi$)の中にある場合，$\xi$の中の$x$
					(ただし$\varphi$で自由に出現している$x$)を全て$\tau$に置き換えた式を
					...
					(resp. $\exists y \widetilde{\xi}(z/y)$)に差し替えた式である．
				...
				\begin{itembox}[l]{IH (帰納法の仮定)}
					...
					\begin{description}
						\item[(1)] $\varphi$における$x$の自由な出現が
							...
							$\forall z \widetilde{\xi}$ 
							を$\forall y \widetilde{\xi}(z/y)$に差し替えた式である．
					...
					\item[case1] $\varphi$が
						...
						\begin{description}
							\item[(1)] $\varphi$における$x$の自由な出現が
								...
								$\forall y \widetilde{\xi}(z/y)$に差し替えた式は，
								(IH)より
								\begin{align}
									\negation \widetilde{\psi}(x/\tau)
								\end{align}
