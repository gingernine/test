\section{拡張}
	...
	\begin{align}
		\varepsilon x A(x)
	\end{align}
	なる形の項を導入する．これはHilbertの{\bf $\varepsilon$項}\index{いぷしろんこう@$\varepsilon$項}
...
\subsection{$\varepsilon$項}
	...
	\begin{itemize}
		...
		\item $\sigma$と$\tau$を$\lang{\varepsilon}$の項とするとき，
			...
			これらは$\lang{\varepsilon}$の{\bf 原子式}\index{げんししき@原子式}
	...
	\begin{screen}
		\begin{metadfn}[$\lang{\varepsilon}$の項の部分項]
	...
	\begin{screen}
		\begin{metadfn}[$\lang{\varepsilon}$の式の項]
	...
	たとえば$\lang{\varepsilon}$の式$\varphi$の上に$\varepsilon$項$e$が現れているとき，
	$e$の真部分項は$\varphi$の項ではない．
	
	\begin{screen}
		\begin{metadfn}[$\lang{\varepsilon}$の部分式]
		\label{metadfn:L_epsilon_subformula}
			$\varphi$を$\lang{\varepsilon}$の式とするとき，
			$\varphi$に現れる$\lang{\varepsilon}$の式のうち，
			$\varphi$に現れる$\varepsilon$項のいずれにも格納されていないものを
	...
	\begin{screen}
		\begin{metadfn}[$\lang{\varepsilon}$の直部分式]
		\label{metadfn:L_epsilon_immediate_subformula}
			$\varphi$を$\lang{\varepsilon}$の式とするとき，$\varphi$の{\bf 直部分式}
			...
			\end{itemize}
			とする．また$\varepsilon x \psi$なる$\varepsilon$項の直部分式は$\psi$とする．
		\end{metadfn}
	\end{screen}
	
	$\lang{\in}$の項と式に関しては，以上の定義と P. \pageref{metadfn:L_in_subterm_of_term}
	の定義は一致する．
	
	\begin{screen}
		\begin{metaaxm}[$\lang{\varepsilon}$の項と式に対する構造的帰納法]
		\label{metaaxm:induction_principle_of_L_epsilon}
			...
			\begin{enumerate}
				...
				\item $\varphi$を任意に与えられた$\lang{\varepsilon}$の式として，
					$\varphi$の任意の項および真部分式に対してXおよびYが言えると仮定するとき，
					\begin{itemize}
	...
	\begin{screen}
		\begin{metadfn}[$\lang{\varepsilon}$の始切片]
		\label{metadfn:L_epsilon_initial_segment}
			$\theta$を$\lang{\varepsilon}$の項或いは式とするとき，
	...
	\begin{screen}
		\begin{metadfn}[$\lang{\varepsilon}$のスコープ]
