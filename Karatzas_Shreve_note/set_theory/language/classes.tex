\section{クラス}
	...
	\begin{screen}
		\begin{dfn}[クラス]
			...
			と$\Set{x}{\varphi(x)}$を{\bf クラス}\index{クラス}{\bf (class)}と呼ぶ．
			またこれらのみがクラスである．
		\end{dfn}
	\end{screen}
	
	クラスには二種類あるので，それらも名前を分けておく．
	\begin{screen}
		\begin{dfn}[主要$\varepsilon$項]
			クラスである$\varepsilon$項を{\bf 主要${\boldsymbol \varepsilon}$項}
			\index{しゅよういぷしんろんこう@主要$\varepsilon$項}
			{\bf (critical epsilon term)}と呼ぶ．
		\end{dfn}
	\end{screen}
	
	\underline{主要$\varepsilon$項はいかなる場合も代入可能である}．
	また後述することだが，本論文における集合とは，主要$\varepsilon$項か
	主要$\varepsilon$項に等しいクラスのことである
	(定理\ref{thm:critical_epsilon_term_is_set})．
	
	
	\begin{screen}
		\begin{dfn}[主要内包項]
			クラスである内包項を{\bf 主要内包項}\index{しゅようないほうこう@主要内包項}と呼ぶ．
		\end{dfn}
	\end{screen}
	...
	主要内包項という呼び名も正則内包項という呼び名も本論文で勝手に付けた名前であるし，
	主要$\varepsilon$項をクラスと呼ぶのも本論文の特徴である．
