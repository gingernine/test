...
\label{sec:Henkin_expansion}
	第\ref{chap:inference}章の証明は，$\lang{\varepsilon}$の文しか使われていない場合に
	{\bf HK}の証明に書き直せることを示す．第\ref{chap:inference}章の証明体系は
	次を公理とした証明体系であり，便宜上{\bf HE}と呼ぶことにする．
	{\bf HK}と違って{\bf HE}の証明に使われる式は全て\underline{文}である．
	\begin{screen}
		\begin{logicalaxm}[{\bf HE}の公理(命題論理)]
			$\varphi$と$\psi$と$\xi$を$\mathcal{L}$の文とするとき
			\begin{description}
	...
	\begin{screen}
		\begin{logicalaxm}[{\bf HE}の公理(量化)]
			$\varphi$を$\mathcal{L}の$式とし，$\tau$を主要$\varepsilon$項とし，
			...
			\begin{description}
				...
				\item[(EE)] $\hat{\varphi}$を$\varphi$か或いは$\varphi$の束縛変項を取り替えた式
					とするとき
					\begin{align}
	...
	\begin{screen}
		\begin{metathm}[主要$\varepsilon$項を変項に置き換えても{\bf HK}の証明]
