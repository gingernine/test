...
\label{sec:Henkin_expansion}
	...
	\begin{screen}
		...
		\label{metathm:Henkin_expansion_2_lemma}
			...
			また$e$をこの列のうち少なくとも一本の式に直部分項として現れる主要$\varepsilon$項とし，
			$y$をこの証明に現れない変項とする．そして各$\varphi_{i}$に対して
			そこに直部分項として現れる$e$を全て$y$に取り替えた式を$\hat{\varphi}_{i}$と書く．
		...
		\begin{description}
			...
			\item[case5] $\varphi_{i}$が前の式$\varphi_{j}$から
				...
				\begin{align}
					\hat{\chi}(x/a)
				\end{align}
				なる式で($e$は主要$\varepsilon$項なので$x$に代入された$a$を含むようには現れない)
				$\hat{\varphi}_{i}$は
				\begin{align}
					\forall x \hat{\chi}
				\end{align}
