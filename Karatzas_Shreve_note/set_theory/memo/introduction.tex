\section{導入}
	...
	そもそも「実在」化の背景には，大元の集合論の世界では集合が一つも「実在」していないという事実がある．
	その大元の，最もシンプルで現代数学の基盤となる集合論は，一階述語論理の言語$\{\in\}$で記述された
	{\bf ZF (Zermelo-Fraenkel)}集合論と呼ばれるものであるが，
	その言語には集合というモノが用意されていないのである．つまり，たとえば
	\begin{align}
		\exists x\, \forall y\, (\, y \notin x\, )
	\end{align}
	は「空集合は存在する」という定理を表しているが，存在するはずの空集合を実際に取ってくることは
	出来ないのである．にもかかわらず多くの場面では空集合$\emptyset$は実在するモノとして扱われているが，
	...
	$\varepsilon$項の中でも特にその導入の意図に適っているものを本稿では主要$\varepsilon$項と呼ぶ．
	この名前自体はHilbertの$\varepsilon$計算でも登場し，
	本来は上述の主要論理式に付属するものであるのだが，
	...
	この理論で第一に特徴的なのは何が集合であるかといった「範囲」が確定することであるが，
	こうした集合は全て有限個の記号を有限個並べたものでしかないから集合は可算個しかない．
	ところが集合の中には実数が存在する筈なので，
	...
	Bourbaki\cite{key5}や島内\cite{key6}でも$\varepsilon$計算を使った集合論を展開している
	(Bourbaki\cite{key5}では$\varepsilon$ではなく$\tau$が使われている)．
	ところで，本稿では$\varepsilon$項だけではなく，「$\varphi(x)$を満たす集合$x$の全体」を表す
	\begin{align}
		\Set{x}{\varphi(x)}
	\end{align}
	なるモノも取り入れる．Bourbaki\cite{key5}や島内\cite{key6}では
	\begin{align}
	...
	なお，類を正式に導入した集合論には他にも{\bf BG (Berneys-G$\ddot{\mbox{o}}$del)}集合論や
	{\bf MK (Morse-Kelly)}集合論や{\bf NF (New Foundations)}がある．
	{\bf BG}集合論と{\bf MK}集合論には変項が二種類あり，
	それぞれ類に対する量化と集合に対する量化に使い分けているが，
	本稿の集合論では類に対する量化は行わない．
	本稿では一階述語論理の範疇で{\bf ZF}集合論の言語に項を追加しただけである．
