\section{導入}
	...
	述語計算の拡張である．$\varepsilon$は式$\varphi(x)$から項$\varepsilon x \varphi(x)$を作るものであり，
	...
	つまり変項が二種類あって，それぞれ類に対する量化と集合に対する量化に使い分けている．
	これに対し本稿では変項は一種類のみ，集合に対するものしか扱わない．
	本稿では一階述語論理の範疇で{\bf ZF}集合論の語彙を拡張しているだけであり，
	{\bf BG}と{\bf MK}が類を量化しているところを本稿では{\bf 図式}\index{ずしき@図式}{\bf (schema)}で対応する．
