\section{導入}
	...
	\index{ていぎによるかくだい@定義による拡大}{\bf (extension by definition)})方法があるが，
	$\varepsilon$を使えば単に
	\begin{align}
	...
	原理的に集合は可算個しか作られていないということになる．集合論の中では実数が存在する筈なのに
	集合が可算個しかないというのは矛盾であるかのように思えるが，
	濃度の話はあくまで式の翻訳の話であって，実在する集合が可算個あれば
	通常の数学が展開出来てしまうのである．実数を扱う場合だって，実数を漏れなく用意
	しなければ議論が破綻するというわけはなく，「実数である」という性質を持つ集合から何が解るかが
	考察の焦点になっているのである．ただし，この理論の集合の概念を受け入れることに抵抗がある場合は，
	空想的な「集合の宇宙」を仮定するのが自然なのかもしれない．
	この理論の特徴として他には，「存在」が「実在」で補完できるために
	...
	これは「集合しか扱わない」という立場にいることが原因であり，集合を拡張した類(class)と呼ばれる
	ものを正式に取り入れれば解決できる．
	存在式の不成立によって項の正体が不明になったり，存在式の不成立によって項が定義できなかったり，
	不成立にもかかわらず非公式に項を定義したり，といったいささか厄介な問題を一気に払拭できて，
	しかもその意図するところを損なわずに$\Set{x}{\varphi(x)}$なる項を
	導入できる簡潔で具体的な手法がある．それは竹内\cite{key4}にあるように
