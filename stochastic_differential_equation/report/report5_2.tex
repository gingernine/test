...
\newpage
	...
	\begin{screen}
		\begin{thm}[$\mathfrak{M}_{2,c}$はHilbert空間]
	...
	\begin{prf}
		...
		\begin{description}
			...
			\item[(3)]
				$N^{(n)} \in \mathcal{M}_{2,c}\ (n=1,2,\cdots)$より
				任意の$s \leq t,\ s,t \in I$と$A \in \mathcal{F}_s$に対して
				\begin{align}
					\int_{A} \cexp{N^{(n_k)}_t}{\mathcal{F}_s}(\omega)\ \mu(d\omega) 
					= \int_{A} N^{(n_k)}_s(\omega)\ \mu(d\omega)
					\quad (k=1,2,\cdots)
				\end{align}
				が成り立つから，H\Ddot{o}lderの不等式及び(\refeq{eq:M_c2_hilbert_2})より
				\begin{align}
					&\left| \int_{A} \cexp{N_t}{\mathcal{F}_s}(\omega)\ \mu(d\omega) - \int_{A} N_s(\omega)\ \mu(d\omega) \right| \\
					&\qquad \leq \left| \int_{A} \cexp{N_t - N^{(n_k)}_t}{\mathcal{F}_s}(\omega)\ \mu(d\omega) \right|
						+ \left| \int_{A} N^{(n_k)}_s(\omega) - N_s(\omega)\ \mu(d\omega) \right| \\
					&\qquad = \left| \int_{A} N_t(\omega) - N^{(n_k)}_t(\omega)\ \mu(d\omega) \right|
						+ \left| \int_{A} N^{(n_k)}_s(\omega) - N_s(\omega)\ \mu(d\omega) \right| \\
					&\qquad \leq \Norm{N_t - N^{(n_k)}_t}{\mathscr{L}^2} + \Norm{N^{(n_k)}_s - N_s}{\mathscr{L}^2}
					\longrightarrow 0 \quad (k \longrightarrow \infty)
				\end{align}
				が従い$\cexp{N_t}{\mathcal{F}_s} = N_s$が得られる．
		\end{description}
		以上より$N$の同値類$\equiv{N}{2,c}$は$\mathfrak{M}_{2,c}$に属する．そして(\refeq{eq:M_c2_hilbert_2})より
		\begin{align}
				\Norm{\equiv{N}{2,c} - \equiv{M^{(n_k)}}{2,c}}{\mathfrak{M}_{2,c}} 
				= \Norm{\equiv{N}{2,c} - \equiv{N^{(n_k)}}{2,c}}{\mathfrak{M}_{2,c}} 
				= \Norm{N_T - N^{(n_k)}_T}{\mathscr{L}^2} \longrightarrow 0 \quad (k \longrightarrow \infty)
		\end{align}
		が成り立つから元のCauchy列も$\equiv{N}{2,c}$に収束する．従って
		$\mathfrak{M}_{2,c}$はHilbert空間である．
		\QED
	\end{prf}
\newpage