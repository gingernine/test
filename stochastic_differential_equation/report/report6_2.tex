\section{可予測過程}
		...
		\begin{lem}[単純可予測過程の性質]\mbox{}
			\begin{description}
				...
				\item[(4)] $\mathcal{S}$は$\semiLp{2}{I \times \Omega,\mathcal{P},\mu_M}$の部分集合である．
					...
					$\mathfrak{S}$から$\Lp{2}{I \times \Omega,\mathcal{P},\mu_M}$への等長\footnotemark
					埋め込みを$J$と表すとき，
	...
	\begin{screen}
		\begin{thm}[左連続適合過程は$\mathcal{P}/\borel{\R}$-可測]
			確率過程$X$が左連続且つ$(\mathcal{F}_t)$-適合なら可測$\mathcal{P}/\borel{\R}$である．
	...
	\begin{prf}
		...
		$X^n$は可測$\mathcal{P}/\borel{\R}$であり，各点収束先にも可測性は保たれる．
		\QED
	\end{prf}
