\section{可予測過程}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				...
				以上で(\refeq{eq:thm_composition_of_predictable_space_measure_0})右辺の積分が定義される．
				
			\item[第二段]
				(\refeq{eq:thm_composition_of_predictable_space_measure_0})で定める$\mu_M$が測度となることを示す．
		...
		\begin{lem}[単純可予測過程の性質]
			...
			\begin{description}
				...
				\item[(3)] 任意の$\mathcal{P}/\borel{\R}$-可測可積分関数$Y$に対し次が成り立つ:
					...
					\end{align}
				\item[(4)] $\mathcal{S}$は$\Norm{\cdot}{\semiLp{2}{\mu_M}}$に関して$\semiLp{2}{\mathcal{P},\mu_M}$の稠密な部分空間である．
					これにより，$\mathfrak{S}$から$\Lp{2}{\mathcal{P},\mu_M}$への等長\footnotemark
	...
	\footnotetext{
		ノルム$\Norm{\cdot}{\Lp{2}{\mu_M}}$に関する等長性を指す．
	}
	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[(2)] 
				(\refeq{eq:dfn_simple_predictable_process})の各項が可測$\mathcal{P}/\borel{\R}$であることを示す．
			...
			\item[(4)]
				...
				$(s,t] \in I$と$F \in \semiLp{\infty}{\Omega,\mathcal{F}_s,\mu}$を取り$F \defunc_{(s,t]}$の二乗可積分性を示す．
				\begin{align}
	...
	\begin{screen}
		\begin{thm}[一様有界な左連続適合過程は$\mathcal{P}/\borel{\R}$-可測]
