\section{$\mathrm{L}^2$における条件付き期待値}
	...
	\item[$\mathrm{L}^2$における内積]
		...
		\end{align}
		まず(\refeq{eq:L2_inner_product})はwell-definedである．実際任意の$f' \in [f]$と$g' \in [g]$に対して
		\begin{align}
		...
		\end{align}
		が成り立ち，H\Ddot{o}lderの不等式より$\inprod<\cdot,\cdot>_{\Lp{2}{\mathcal{F}}}$
		...
		\begin{description}
			\item[正値性] 
				...
				により従う．
			\item[対称性] 
				(\refeq{eq:L2_inner_product})において被積分関数を$gf$としても積分値は変わらない．
			\item[双線型性] 
	...
	\item[条件付き期待値の存在]
		...
		$\Lp{2}{\Omega, \mathcal{G},\mu}$の完備性より
		$J_{\mathcal{G}} \Lp{2}{\Omega, \mathcal{G},\mu}$は$\Lp{2}{\Omega, \mathcal{F},\mu}$の閉部分空間となる．
		任意の$[f]_{\mathcal{F}} \in \Lp{2}{\Omega, \mathcal{F},\mu}$に対して
		$J_{\mathcal{G}} \Lp{2}{\Omega, \mathcal{G},\mu}$への射影を対応させる写像を$P_\mathcal{G}$と表し，
		また単射$J_{\mathcal{G}}$の値域を$J_{\mathcal{G}} \Lp{2}{\Omega, \mathcal{G},\mu}$に制限した全単射を$T_{\mathcal{G}}$と表す．
		この$P_{\mathcal{G}}$と$T_{\mathcal{G}}^{-1}$の合成写像
		\begin{align}
			T_{\mathcal{G}}^{-1} P_{\mathcal{G}}:\Lp{2}{\Omega, \mathcal{F},\mu} \longrightarrow \Lp{2}{\Omega, \mathcal{G},\mu}
	...
	\begin{screen}
		\begin{dfn}[$\mathrm{L}^2$における条件付き期待値作用素]
			(\refeq{eq:composition_L2_conditional_expectation})で定めた$T_{\mathcal{G}}^{-1} P_{\mathcal{G}}$を
			\begin{align}
			...
			\end{align}
			と表現し，$\mathcal{G}$で条件付けた条件付き期待値(conditional expectation)と呼ぶ．
