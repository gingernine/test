...
\newpage
	...
	\begin{prf}[レポート問題6の続き]
		\begin{description}
			\item[第一段] $N \in \mathcal{M}_{c,loc}$を示す．
				(\refeq{eq:Ito-Integral_on_M_c_loc_limit})により定めた$I_M(X)$は適合過程であり，
			...
			\item[第二段]
				...
				今，任意に$j \in \N_0$を取り固定する．(\refeq{eq:thm_left_continuous_adapted_then_predictable})に倣い
			...
			\item[第三段]
				(\refeq{eq:Ito-Integral_on_M_c_loc_report_7_3})を示す．
				\QED
		\end{description}
	\end{prf}