	\begin{screen}
		\begin{thm}[二次変分の存在]
			...
			\end{align}
			特に$(\tau_j)_{j=0}^{\infty} \in \mathcal{T}$に対し$M^{\tau_j} \in \mathcal{M}_{b,c}\ (j=0,1,\cdots)$
			となるとき，$\left( M^{\tau_j} \right)^2 - A^{\tau_j}  \in \mathcal{M}_{b,c}\ (j=0,1,\cdots)$が満たされる．
			$A' \in \mathcal{A}^+$もまた上の主張を満たすときは$A$と$A'$は$\mu$-a.s.にパスが一致し，
			逆に$A,A' \in \mathcal{A}^+$が$\mu$-a.s.にパスが一致するならば$A'$も主張を満たす．
			また$M \in \mathcal{M}_{b,c}$の場合は$A_0 = 0\ \mbox{$\mu$-a.s.}$且つ
			$M^2 - A \in \mathcal{M}_{b,c}$を満たす
			$A \in \mathcal{A}^+$が存在する．
