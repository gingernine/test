...
\begin{frame}\frametitle{古典論理}
	...
	\begin{alertblock}{{\bf HK}の論理的公理(命題論理)}
		\begin{description}
			...
			\item[矛盾の導入1] $\varphi \rarrow (\, \negation \varphi \rarrow \bot\, ).$
			\item[矛盾の導入2] $\negation \varphi \rarrow (\, \varphi \rarrow \bot\, ).$
			\item[否定の導入] $(\, \varphi \rarrow \bot\, ) \rarrow\ \negation \varphi.$
			\item[論理和の導入1] $\varphi \rarrow \varphi \vee \psi.$
			\item[論理和の導入2] $\psi \rarrow \varphi \vee \psi.$
			...
			\item[論理積の除去1] $\varphi \wedge \psi \rarrow \varphi.$
			\item[論理積の除去2] $\varphi \wedge \psi \rarrow \psi.$
