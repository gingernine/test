...
\begin{frame}\frametitle{クラスについて}
	\begin{itemize}
		...
		%\item 集合ではない「モノの集まり」も，``合法的''に集合のように扱いたい．
		\item クラスを扱う理論として{\bf BG}や{\bf MK}がある．これらはクラス用の変項(大文字)を用意して，
			...
			(G$\ddot{\mbox{o}}$del\cite{Godel}，Morse\cite{Morse}，Fraenkel,Bar-Hillel$\&$Levy\cite{BarHillel})．
...
\begin{frame}\frametitle{古典論理}
	...
	\begin{exampleblock}{{\bf HK}の証明(参考：戸次\cite{Bekki})}
		...
		$\varphi_{n} \equiv \psi$で，各$\varphi_{i}$が次のいずれかであるもの：
		\begin{itemize}
			\item {\bf HK}の公理である
			\item $\Gamma$の公理である
...
\begin{frame}\frametitle{$\Sigma$の公理}
	...
	\begin{itemize}	
		...
		\item 要素の公理のココロはG$\ddot{\mbox{o}}$del\cite{Godel}から引用．
	\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{{\bf HE}の証明}
	...
	\begin{exampleblock}{{\bf HE}の証明}
		「$\Sigma$から$\psi$への{\bf HE}の証明で$\mathcal{L}$の文の列であるもの」とは，
...
\begin{frame}\frametitle{まとめ}
	\begin{itemize}
		\item 本論文では$\varepsilon$項とクラスの直接的な導入を併せた集合論を構築した．
	...
	\begin{block}{何が具体的で直観的か}
		\begin{itemize}
			...
			\item 存在するなら取って来られる．
			%\item クラスの取り入れ方
			%\itemm 変項で語るのではなく集合を直に扱っているという感覚に順応
		\end{itemize}
	\end{block}
\end{frame}
