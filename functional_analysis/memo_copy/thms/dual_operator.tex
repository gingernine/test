\section{ノルム空間の共役作用素}
	...
	\begin{screen}
		\begin{dfn}[共役作用素]
			$X,Y$をノルム空間，$T:X \oparrow Y$を線型作用素とし，
			$\closure{\Dom{T} } = X$を満たすとする．
			\begin{align}
				\mathcal{D} \coloneqq \Set{g \in Y^*}{\mbox{或る$f \in X^*$が存在して$f(x) = g(Tx) \quad (\forall x \in \Dom{T} ) $を満たす．}}
				\label{eq:dfn_dual_operator}
			\end{align}
			と定めれば，$g \in \mathcal{D}$に対して
			(\refeq{eq:dfn_dual_operator})の関係を満たす
			$f$の存在は一意的であり\footnotemark
			この対応を
			\begin{align}
				T^*: \mathcal{D} \ni g \longmapsto f
			\end{align}
	...
	この定理を証明するために以下にいくつか準備をする．$x \in X$と$f \in X^*$に対して$f(x)$を次の双線型形式で表現する:
	\begin{align}
		\inprod<x,f>_{X,X^*} \coloneqq f(x).
	\end{align}
	双線型形式で表現することで内積空間を扱っているように捉えることができ，
	例えば(\refeq{eq:dfn_dual_operator})において
	\begin{align}
