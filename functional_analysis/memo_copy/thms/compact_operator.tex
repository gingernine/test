	...
	\begin{prf}
		...
		\begin{description}
			\item[第一段] $T$が$\mathrm{L}^p(\nu)$から$\mathrm{L}^p(\mu)$への有界線型作用素であることを示す．
				...
				は$\mu$-零集合である
				\footnote{
					Fubiniの定理より$x \longmapsto \int_Y |K(x,y)|\ \nu(dy)$は可測$\mathcal{M}/\borel{[-\infty,\infty]}$である．
				}
			...
			\item[第二段] $K$を単関数で近似できることを示す．
				...
				\begin{align}
					...
					< \frac{\epsilon}{3}
				\end{align}
				を満たす．$K_N$は$(X_N \times Y_N)^c$上で0であるから，$K_N$の単関数近似列$(K_N^n)_{n=1}^{\infty}$は
				\begin{align}
					K_N^n = \sum_{i=0}^{m_n} \alpha_{n,i} \defunc_{E_{n,i}}
					\quad (\alpha_{n,i} \in \C,\ E_{n,i} \in \mathcal{M} \times \mathcal{N},\ \alpha_{n,0} = 0,\ X_N \times Y_N \subset E_{n,0})
				\end{align}
				を満たすように構成できる．そして或る$N' \in \N$が存在して
				\begin{align}
					\Norm{K_N - K_N^{N'}}{\mathrm{L}^q(\mu \times \nu)} < \frac{\epsilon}{3} 
				\end{align}
				を満たす．また$\mu \times \nu(E) < \infty$を満たす任意の$E \in \mathcal{M} \times \mathcal{N}$と
				任意の$\eta > 0$に対し，或る$A_1,A_2,\cdots \in \mathcal{M}$と$B_1,B_2,\cdots \in \mathcal{N}$が存在して
				\begin{align}
					E \subset \bigcup_{j=1}^{\infty} A_j \times B_j,
					\quad \sum_{j=1}^{\infty} \mu(A_j)\nu(B_j) < \mu \times \nu (E) + \frac{\eta}{2}
				\end{align}
				が成り立つようにでき，さらに或る$N'' \in \N$が存在して
				\begin{align}
					\bigcup_{j=N'' + 1}^{\infty} \mu(A_j)\nu(B_j) < \frac{\eta}{2}
				\end{align}
				を満たすから，
				\begin{align}
					E_\eta \coloneqq \bigcup_{j=1}^{N''} A_j \times B_j
				\end{align}
				とおけば
				\begin{align}
					\mu \times \nu(E \cap E_\eta^c) + \mu \times \nu(E^c \cap E_\eta)
					\leq \mu \times \nu\left(\bigcup_{j=N''+1}^{\infty} A_j \times B_j\right)
						+ \mu \times \nu\left(\bigcup_{j=1}^{\infty} A_j \times B_j \backslash E\right)
					< \eta
				\end{align}
				が成り立つ．従って$K_N^{N'}$に対し$\defunc_{A \times B}\ (A \in \mathcal{M},B \in \mathcal{N})$
				の形の線型結合で表される関数$\tilde{K}$が存在して
				\begin{align}
					\Norm{K_N^{N'} - \tilde{K}}{\mathrm{L}^q(\mu \times \nu)} < \frac{\epsilon}{3}
				\end{align}
				を満たす．よって
				\begin{align}
					\Norm{K - \tilde{K}}{\mathrm{L}^q(\mu \times \nu)} < \epsilon
				\end{align}
				が成り立つ．$\epsilon$は任意であったから，任意の$v \in \N$に対して或る$\tilde{K}_v$が存在し，
				\begin{align}
					\Norm{K - \tilde{K}_v}{\mathrm{L}^q(\mu \times \nu)} < \frac{1}{v}
				\end{align}
				を満たし，$\tilde{K}_v$は
				\begin{align}
					\tilde{K}_v = \sum_{i=0}^{M_v} \beta_{v,i} \defunc_{A_{v,i} \times B_{v,i}}
					\quad (\beta_{v,i} \in \C,\ A_{v,i} \in \mathcal{M},B_{v,i} \in \mathcal{N},\ \beta_{v,0} = 0,\ X_N \times Y_N \subset A_{v,0} \times B_{v,0})
				\end{align}
				の形で表現できる．$T_v$を$\tilde{K}_v$を積分核とする作用素とすれば，任意の$f \in \mathrm{L}^p(\nu)$に対して
				\begin{align}
					T_v f (x) = \int_Y \sum_{i=0}^{M_v} \beta_{v,i} \defunc_{A_{v,i} \times B_{v,i}}(x,y) f(y)\ \nu(dy)
					= \sum_{i=0}^{M_v} \beta_{v,i} \int_{B_{v,i}} f(y)\ \nu(dy) \defunc_{A_{v,i}}(x)
					\quad (x \in X)
				\end{align}
				となり，$T_v Y$は有限次元空間である.
				\QED
		\end{description}
	\end{prf}
