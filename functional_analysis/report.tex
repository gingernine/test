...
約束及び定義(問題用紙引用)
\begin{itemize}
	\item 係数体は，特に断らない限り，複素数体$\C$とする．$\N_0=\{0,1,2,\cdots\}$．
	...
	\item ノルム空間$X$と$Y$が同型$\quad \xLeftrightarrow[]{\mathrm{def}} \quad$線形全単射$T\ :\ X \rightarrow Y$で$\Norm{Tx}{} = \Norm{x}{}\ (\forall x \in X)$
		なるものが存在する．($T$をノルム空間としての同型写像という．同様にBanach空間に関する同型の概念を定める．)
	\item $s$を複素数列全体のなす線形空間とし，$s$の部分集合を以下のように定める：
		\begin{align}
			&l^p = \{a = (a_n)_{n=1}^{\infty} \in s\ ;\ \Norm{a}{l^{p}} = ( \sum_{n=1}^{\infty}|a_n|^p )^{1/p} < \infty \},\ 1 \leq p < \infty \\
			&l^{\infty} = \{a = (a_n)_{n=1}^{\infty} \in s\ ;\ \Norm{a}{l^{\infty}} = \sup{n}{|a_n|} < \infty\}, \quad
			c_0=\{a=(a_n)_{n=1}^{\infty} \in s\ ;\ \lim_{n \to \infty}a_n=0\}.
		\end{align}
		このとき$l^p$は$\Norm{\cdot}{l^p}$をノルムとしてBanach空間である．
\end{itemize}
...
\begin{description}\mbox{}
	...
	\item[(2)]
		...
		\end{align}
		の関係がまず示される．後は式(\refeq{eq:func_analy_Q5_2})右辺のinfを実現する$g \in Y$の存在を言えばよい．例えば区間$[a,\ 1]$上で$f$と$|f(a)|$の距離を保ち続ける
		...
		式(\refeq{eq:func_analy_Q5_1})により$T$は$\Norm{[f]}{X/Y} = \Norm{f|_{[0,\ a]}}{} = \Norm{T[f]}{}$の意味で等長である．
		そして$T$の線型性は
		\begin{align}
...
\begin{description}\mbox{}
	...
	\item[(2)] まず，$l^1$は$\Norm{\cdot}{l^1}$をノルムとしてBanach空間となり，$c_0^*$は$T_a$の値域$\C$がBanach空間であるから作用素ノルム
		...
		$n \leq M$を満たす少なくとも一つの$n \in \N$については$a_n \neq 0$となるように十分大きいところの$M \in \N$を取って，この$M$に対して$x = (x_n) \in c_0$として
		\begin{align}
			x_n = \begin{cases}
				\overline{a_n}/|a_n| & a_n \neq 0,\rm{and}\ n \leq M \\
				0 & a_n = 0,\rm{or}\ n > M
...
\begin{prf}
\begin{description} $l^p$の共役空間，第二共役空間の作用素ノルムをそれぞれ$\Norm{\cdot}{\left(l^p\right)^*},\ \Norm{\cdot}{\left(l^p\right)^{**}}$と表す．
	\item[$l^p$の共役空間] まず$l^p$の共役空間の任意の元$f \in \left(l^p\right)^*$が或る$v = (v_n)_{n=1}^{\infty} \in l^q\ (1/p + 1/q = 1)$に対応して
		\begin{align}
			f(x) = \sum_{n=1}^{\infty} x_n v_n, \quad \left(x = (x_n)_{n=1}^{\infty} \in l^p\right)
		\end{align}
		で表現されることを示す．Kroneckerのデルタを用いて
		\begin{align}
			e_n \coloneqq (\delta_{jn})_{j=1}^{\infty}
		\end{align}
		で表現される$e_n\ (n=1,2,3,\cdots)$は$l^p$の元であり，各$n = 1,2,3,\cdots$に対して
		\begin{align}
			v_n \coloneqq f(e_n)
		\end{align}
		とおく．任意の$x=(x_n) \in l^p$に対して
		\begin{align}
			x^{(N)} = \sum_{n=1}^{N} x_n e_n, \quad (N = 1,2,3,\cdots)
		\end{align}
		として作られる数列の族$\left(x^{(N)}\right)_{N=1}^{\infty}$は$l^p$において収束し$x^{(N)} \rightarrow x\ (N \rightarrow +\infty)$が成り立つ．
		これは級数の収束を考えれば
		\begin{align}
			\Norm{x - x^{(N)}}{l^p}^p = \sum_{n=N+1}^{+\infty} |x_n|^p \longrightarrow 0 \quad (N \longrightarrow +\infty)
		\end{align}
		が成り立つからである．
		$f$が有界線型汎関数であることも併せれば
		\begin{align}
			\left|f(x) - f(x^{(N)})\right| \leq \Norm{f}{\left(l^p\right)^*}\Norm{x - x^{(N)}}{l^p} \longrightarrow 0\quad (N \longrightarrow +\infty) 
		\end{align}
		により
		\begin{align}
			f(x) = \lim_{N \to +\infty} f(x^{(N)}) 
			= \lim_{N \to +\infty} \sum_{n=1}^{N} f(x_n e_n)
			= \lim_{N \to +\infty} \sum_{n=1}^{N} x_n v_n 
			= \sum_{n=1}^{+\infty} x_n v_n \quad (\forall x \in l^p) \label{eq:func_analy_Q13_3}
		\end{align}
		が成り立つ．後は上で定義した複素数列$v = (v_n)_{n=1}^{\infty}$が$l^q$に属していることを示せばよい．
		$v = 0$の場合は明らかに$v \in l^q$であるから$v \neq 0$である場合を考える．
		各$n \in \N$について
		\begin{align}
			v_n = |v_n| \exp{\sqrt{-1} \theta_n}
		\end{align}
		が成り立つように$\theta_n \in \R^1$を一つ選ぶ．$v = (v_n)_{n=1}^{+\infty}$に対して，$n \leq N$を満たす少なくとも一つの$n \in \N$
		については$v_n \neq 0$となるように，十分大きいところの$N \in \N$を任意に取り，
		\begin{align}
			w_n = \begin{cases}
				|v_n|^{q-1} \exp{- \sqrt{-1} \theta_n} & (n = 1,2,\cdots,N), \\
				0 & (n = N+1, N+2, \cdots)
			\end{cases} 
		\end{align}
		として複素数列$w = (w_n)_{n=1}^{+\infty}$を作る．有限個を除いて0となる数列だから$w \in l^p$であり，
		\begin{align}
			\Norm{w}{l^p}^p = \sum_{n=1}^{N} |w_n|^p = \sum_{n=1}^{N} |v_n|^q
		\end{align}
		が成り立っていることにより，式(\refeq{eq:func_analy_Q13_3})の結果と合わせて
		\begin{align}
			f(w) = \sum_{n=1}^{+\infty} w_n v_n = \sum_{n=1}^{N} |v_n|^q 
			= \left( \sum_{n=1}^{N} |v_n|^q \right)^{\frac{1}{q}}\left( \sum_{n=1}^{N} |v_n|^q \right)^{\frac{1}{p}}
			= \left( \sum_{n=1}^{N} |v_n|^q \right)^{\frac{1}{q}} \Norm{w}{l^p}
		\end{align}
		と表現できる．$f$が有界線型作用素であることにより
		\begin{align}
			\left( \sum_{n=1}^{N} |v_n|^q \right)^{\frac{1}{q}} \Norm{w}{l^p} \leq \Norm{f}{\left(l^p\right)^*} \Norm{w}{l^p}
		\end{align}
		となるから，十分大きな任意の$N \in \N$について
		\begin{align}
			\sum_{n=1}^{N} |v_n|^q \leq \Norm{f}{\left(l^p\right)^*}^q
		\end{align}
		が成り立つことになるから，
		\begin{align}
			\sum_{n=1}^{+\infty} |v_n|^q \leq \Norm{f}{\left(l^p\right)^*}^q
		\end{align}
		が示され$v \in l^q$であることがわかる．
	\item[\rm{(i)} $\Rightarrow$ \rm{(ii)}]
		...
		\end{align}
		が成り立つことになる．$l^p \ni x \longmapsto T_x \in \left(l^p\right)^{**}$を用いれば，式(\refeq{eq:func_analy_Q13_1})は
		\begin{align}
		...
		\end{align}
		が成り立つことになる．これは式(\refeq{eq:func_analy_Q13_2})によって
		\begin{align}
