\section{緩増加超関数の構造定理}
	...
	\begin{prf}
		...
		\end{align}
		により$W$は線型性をもち，かつ(\refeq{eq:lem_isomorphism_on_product_of_dual_spaces_2})より
		\begin{align}
	...
	\begin{screen}
		\begin{thm}[緩増加超関数の合成積の性質]
			$u \in \tempdist{\R^n},\ \varphi \in \rapid{\R^n}$とする．
			\begin{description}
				\item[(1)] 合成積$u \ast \varphi$に対し或る緩増加関数$f$が存在して
					$u \ast \varphi = u_f$を満たす．
					
				\item[(2)] $\ft{u \ast \varphi} = \ft{u} \ft{\varphi}$.
				\item[(3)] $\ft{u \varphi} = (2\pi)^{-n} \ft{u} \ast \ft{\varphi}.$
			\end{description}
		\end{thm}
	\end{screen}
