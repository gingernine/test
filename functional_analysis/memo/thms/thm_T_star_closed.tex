...
\section{ノルム空間の共役作用素}
	...
	\begin{screen}
		\begin{dfn}[共役作用素]
			...
			$T$の定義域$\mathscr{D}(T)$が$X$で稠密であるとき，$g \in Y^*$に対し
			\begin{align}
				f(x) = g(Tx) \quad (\forall x \in \mathscr{D}(T)) \label{eq:dfn_dual_operator}
			\end{align}
	...
	\footnotetext{
		...
		が存在すれば
		\begin{align}
			f(x) = f'(x) \quad (\forall x \in \mathscr{D}(T))
		\end{align}
		が成り立つ．$\mathscr{D}(T)$は$X$で稠密であるから
		$f,f'$の連続性より$f = f'$が従う．
	}
	...
	\begin{prf}
		(\refeq{eq:thm_T_star_closed})より$\mathscr{G}\left( \closure{T}^* \right) = \varphi\left[U\mathscr{G}(\closure{T}) \right]^{\perp}$
		が成り立っているから，
		\begin{align}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[必要性]
				(\refeq{eq:thm_T_star_closed})より，$\closure{\mathscr{D}(T)}  = X$ならば
	...
	\begin{prf}
		...
		が成り立つ
		\footnotetext{
			$\mathscr{D}(T + U) \subset \mathscr{D}(T),\mathscr{D}(U)$である．
		}
