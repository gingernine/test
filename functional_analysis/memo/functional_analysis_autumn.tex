...
\def\Dom#1 {\mathscr{D}\left(#1\right)} %作用素の定義域
\def\Graph#1 {\mathscr{G}\left( #1 \right)} %作用素のグラフ
\def\Ran#1{\mathscr{R}\left( #1 \right)} %作用素の値域
\def\Kar#1{\mathscr{N}\left( #1 \right)} %作用素の核
\def\Bop#1#2{\mathrm{B}\left(#1,#2\right)} %有界作用素の空間
\def\selfBop#1{\mathrm{B}\left(#1\right)} %有界作用素の空間[始集合=終集合]
\def\Cop#1#2{\mathrm{B}_c\left(#1,#2\right)} %コンパクト作用素の空間
\def\selfCop#1{\mathrm{B}_c\left(#1\right)} %コンパクト作用素の空間[始集合=終集合]
...
\def\c#1{{\bf C}(#1)} %有界実連続関数
\def\cbound#1{{\bf C_{b}} (#1)} %有界実連続関数
...
\input{appendixes/quotient_normed_space}
\newpage
\printindex
%
%
