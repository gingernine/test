...
\def\Dom#1 {\mathcal{D}( #1 )} %作用素の定義域
\def\Graph#1 {\mathcal{G}( #1 )} %作用素のグラフ
\def\Ran#1{\mathcal{R}( #1 )} %作用素の値域
\def\Ker#1{\mathcal{N}( #1 )} %作用素の核
...
\def\cvan#1{C_\infty (#1)} %コンパクトな台を持つ連続関数
