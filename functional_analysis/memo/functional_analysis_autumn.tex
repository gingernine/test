...
\def\ft#1{\mathfrak{F}#1} %Fourier変換
\def\cft#1{\overline{\mathfrak{F}}#1} %逆Fourier変換
\def\c#1{C(#1)} %連続関数
...
%
%本文
\input{thms/normed_space}
\input{thms/quotient_normed_space}
\chapter{閉作用素}
	\input{thms/closed_linear_operator}
\chapter{Bochner積分}
	\input{thms/Bochner_integral}
	\input{thms/Bochner_integral_2}
	\input{thms/Bochner_integral_3}
\chapter{共役作用素}
	\input{thms/dual_operator}
	\input{thms/dual_operator_2}
\chapter{レゾルベントとスペクトル}
	\input{thms/resolvent_spectral}
	
\input{thms/compact_operator}
\input{thms/compact_operator_2}
\input{thms/compact_operator_3}
\input{thms/compact_operator_4}
\chapter{自己共役作用素のスペクトル分解}
	\input{../../W_Rudin_analysis/chapters/complex_measures}
	\input{thms/complex_measures_integration}
	\input{thms/complex_measures_integration_2}
	\input{thms/spectral_decomposition_of_self_adjoint_operator}
	\input{thms/spectral_decomposition_of_self_adjoint_operator_2}
	\input{thms/spectral_decomposition_of_self_adjoint_operator_3}
...
\chapter{垣田高夫「シュワルツ超関数入門」メモ}
	...
	以降，線型空間は全て複素係数と考える．
	\input{thms/distributions_2}
	\input{thms/distributions_0}
	\input{thms/distributions_1}
	\input{thms/distributions_3}
\appendix
\input{appendixes/weak_convergence}
\input{appendixes/Riemann_series}
\newpage
\printindex
%
%
